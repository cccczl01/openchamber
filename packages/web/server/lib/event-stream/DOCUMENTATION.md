...
## Purpose
This module contains the NoatinWork message-stream WebSocket protocol and runtime bridge. It keeps the browser-facing WebSocket transport separate from the upstream OpenCode SSE transport.
...
- NoatinWork still fetches OpenCode upstream event streams over SSE.
...
- Global synthetic events such as `noatinwork:session-status`, `noatinwork:session-activity`, `noatinwork:notification`, and `noatinwork:heartbeat` are preserved on the WS path, but heartbeat frames are emitted only while an upstream SSE stream is actively attached.

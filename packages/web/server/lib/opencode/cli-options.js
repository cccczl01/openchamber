export const parseServeCliOptions = ({
  ...
  const envPassword =
    env.NOATINWORK_UI_PASSWORD ||
    env.OPENCODE_UI_PASSWORD ||
    null;
  const envCfTunnel = env.NOATINWORK_TRY_CF_TUNNEL === 'true';
  const envTunnelProvider = env.NOATINWORK_TUNNEL_PROVIDER || undefined;
  const envTunnelMode = env.NOATINWORK_TUNNEL_MODE || undefined;
  const envTunnelConfigRaw = env.NOATINWORK_TUNNEL_CONFIG;
  const envTunnelConfig = typeof envTunnelConfigRaw === 'string'
    ? (envTunnelConfigRaw.trim().length > 0 ? envTunnelConfigRaw.trim() : null)
    : undefined;
  const envTunnelToken = env.NOATINWORK_TUNNEL_TOKEN || undefined;
  const envTunnelHostname = env.NOATINWORK_TUNNEL_HOSTNAME || undefined;
  const envApiOnly = env.NOATINWORK_API_ONLY === '1' || env.NOATINWORK_API_ONLY === 'true';

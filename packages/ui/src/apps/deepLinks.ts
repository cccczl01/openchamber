/**
 * NoatinWork deep-link vocabulary — the single source of truth for the `noatinwork://`
...
export const DEEP_LINK_SCHEME = 'noatinwork';
...
const segmentsOf = (url: URL): string[] => {
  // Custom-scheme URLs put the first route token in `host` (noatinwork://session/<id>),
  // but be tolerant of authority-less forms (noatinwork:/session/<id>) where it lands in
...
/**
 * Parse a raw `noatinwork://…` string into a typed intent, or `null` if it isn't a
 * recognised NoatinWork deep link. Tolerant by design: unknown routes return `null`
...
/**
 * Build a canonical `noatinwork://…` URL for an intent. Used by anything that needs to hand

...
export const isUnsafeUnauthenticatedLanAllowed = (env = process.env) =>
  env?.NOATINWORK_ALLOW_UNAUTHENTICATED_LAN === 'true';

export const getUnauthenticatedLanErrorMessage = (host) =>
  `NoatinWork refuses to bind to ${host || 'a network-exposed host'} without UI authentication. `
  + 'Set --ui-password or NOATINWORK_UI_PASSWORD before exposing it over LAN, '
  + 'or set NOATINWORK_ALLOW_UNAUTHENTICATED_LAN=true to accept the risk.';

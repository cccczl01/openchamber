...
export const registerScheduledTaskRoutes = (app, dependencies) => {
  ...
  app.get('/api/noatinwork/scheduled-tasks/status', async (_req, res) => {
    try {
  ...
  app.get('/api/noatinwork/events', (req, res) => {
    ...
    const clients = getNoatinWorkEventClients();
    clients.add(res);

    try {
      writeSseEvent(res, {
        type: 'noatinwork:event-stream-ready',
        properties: {

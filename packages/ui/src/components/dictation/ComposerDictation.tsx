...
export const ComposerDictation: React.FC<ComposerDictationProps> = ({
    ...
    // The dictation server (WebSocket + STT worker) lives in the NoatinWork
    ...
    React.useEffect(() => {
        ...
        window.addEventListener('noatinwork:dictation-toggle', onToggle);
        return () => window.removeEventListener('noatinwork:dictation-toggle', onToggle);

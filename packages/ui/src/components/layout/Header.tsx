...
export const Header: React.FC<HeaderProps> = ({
  ...
  const checkRemoteInstanceUpdate = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await runtimeFetch(`/api/noatinwork/update-check?${params.toString()}`, {
        method: 'GET',
  ...
  const currentSessionTitle = React.useMemo(() => {
    if (!currentSessionId) {
      return activeProjectLabel ?? 'NoatinWork';
    }
  ...
  useEffect(() => {
    ...
    void syncFullscreenState();
    window.addEventListener('noatinwork:window-resized', onResize);

    return () => {
      disposed = true;
      window.removeEventListener('noatinwork:window-resized', onResize);
    };

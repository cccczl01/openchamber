...
4. The **relay** (`noatinwork-website/apps/api`, Cloudflare Worker) verifies the signature +
...
- **Relay (default):** server has no Apple key; `NOATINWORK_PUSH_RELAY_URL` defaults to
  `https://api.noatinwork.dev/v1/push/send` (register URL is derived as `…/register-token`).
- **Direct (fallback):** set `NOATINWORK_PUSH_RELAY_DISABLED=true` + `NOATINWORK_APNS_KEY_ID/
  TEAM_ID/P8` to sign+send from the server itself (HTTP/2 + ES256 JWT); no relay binding needed.

## Config

Server (`apns-runtime.js`):
- `NOATINWORK_PUSH_RELAY_URL` (default the public relay), `NOATINWORK_APNS_ENVIRONMENT`
...
- Direct fallback: `NOATINWORK_APNS_KEY_ID`, `NOATINWORK_APNS_TEAM_ID`, `NOATINWORK_APNS_P8`
  (or `_P8_PATH`), `NOATINWORK_APNS_BUNDLE_ID`, `NOATINWORK_PUSH_RELAY_DISABLED=true`.
...
1. Apple **Keys** (not Certificates) → create an **APNs Auth Key** (`.p8`) → Key ID + Team ID;
   enable **Push Notifications** on App ID `com.noatinwork.app`.
2. In the **noatinwork-website** repo → Actions secrets: `APNS_P8` (PEM), `APNS_KEY_ID`,

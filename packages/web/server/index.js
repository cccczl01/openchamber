...
import { registerNoatinWorkRoutes } from './lib/opencode/noatinwork-routes.js';
...
import { createNoatinWorkSessionService } from './lib/noatinwork-sessions/routes.js';
import { createScheduledTaskService } from './lib/scheduled-tasks/service.js';
import { createNoatinWorkControlService } from './lib/noatinwork-control/service.js';
...
const DESKTOP_NOTIFY_PREFIX = '[NoatinWorkDesktopNotify] ';
const uiNotificationClients = new Set();
const uiNotificationWsClients = new Set();
const uiNoatinWorkEventClients = new Set();
...
const SSE_PATH_PREFIXES = [
  ...
  '/api/noatinwork/events',
  '/api/noatinwork/realtime-proxy/sse',
];

function shouldSkipCompression(req, res) {
  if (process.env.NOATINWORK_RUNTIME === 'desktop') {
    return true;
  }
...
const shouldSkipApiCompression = () => {
  if (isEnvFlagEnabled(process.env.NOATINWORK_SKIP_API_COMPRESSION)) return true;
  if (isEnvFlagEnabled(process.env.NOATINWORK_COMPRESS_API)) return false;
  if (isEnvFlagDisabled(process.env.NOATINWORK_COMPRESS_API)) return true;
  return process.env.NOATINWORK_RUNTIME === 'desktop';
};

const NOATINWORK_VERBOSE_REQUEST_LOGS = isEnvFlagEnabled(process.env.NOATINWORK_VERBOSE_REQUEST_LOGS);
...
const NOATINWORK_USER_CONFIG_ROOT = path.join(os.homedir(), '.config', 'noatinwork');
const NOATINWORK_USER_THEMES_DIR = path.join(NOATINWORK_USER_CONFIG_ROOT, 'themes');
const NOATINWORK_PROJECTS_CONFIG_DIR = path.join(NOATINWORK_USER_CONFIG_ROOT, 'projects');
...
const themeRuntime = createThemeRuntime({
  fsPromises,
  path,
  themesDir: NOATINWORK_USER_THEMES_DIR,
...
const NOATINWORK_DATA_DIR = process.env.NOATINWORK_DATA_DIR
  ? path.resolve(process.env.NOATINWORK_DATA_DIR)
  : path.join(os.homedir(), '.config', 'noatinwork');
const SETTINGS_FILE_PATH = path.join(NOATINWORK_DATA_DIR, 'settings.json');
const PUSH_SUBSCRIPTIONS_FILE_PATH = path.join(NOATINWORK_DATA_DIR, 'push-subscriptions.json');
const APNS_TOKENS_FILE_PATH = path.join(NOATINWORK_DATA_DIR, 'apns-tokens.json');
const REMOTE_CLIENTS_FILE_PATH = path.join(NOATINWORK_DATA_DIR, 'remote-clients.json');
const CLIENT_PAIRING_SESSIONS_FILE_PATH = path.join(NOATINWORK_DATA_DIR, 'client-pairing-sessions.json');
const CLOUDFLARE_MANAGED_REMOTE_TUNNELS_FILE_PATH = path.join(NOATINWORK_DATA_DIR, 'cloudflare-managed-remote-tunnels.json');
const CLOUDFLARE_LEGACY_NAMED_TUNNELS_FILE_PATH = path.join(NOATINWORK_DATA_DIR, 'cloudflare-named-tunnels.json');
...
const projectConfigRuntime = createProjectConfigRuntime({
  fsPromises,
  path,
  projectsDirPath: NOATINWORK_PROJECTS_CONFIG_DIR,
});
...
const ENV_SKIP_OPENCODE_START = process.env.OPENCODE_SKIP_START === 'true' ||
                                    process.env.NOATINWORK_SKIP_OPENCODE_START === 'true';
const ENV_DESKTOP_NOTIFY = (() => {
  if (process.env.NOATINWORK_DESKTOP_NOTIFY === 'true') {
    return true;
  }
  ...
  return /noatinwork-server/i.test(argv0) || /noatinwork-server/i.test(argv1);
})();
...
const ENV_CONFIGURED_API_PREFIX = normalizeApiPrefix(
  process.env.OPENCODE_API_PREFIX || process.env.NOATINWORK_API_PREFIX || ''
);
...
const processForwardedEventPayload = (payload, emitSyntheticEvent) => {
  ...
  emitSyntheticEvent({
    type: 'noatinwork:session-status',
    properties: {
      sessionID: sessionId,
      status,
      timestamp: Date.now(),
  ...
  emitSyntheticEvent({
    type: 'noatinwork:session-activity',
    properties: {
      sessionId,
...
const scheduledTasksRuntime = createScheduledTasksRuntime({
  ...
  emitTaskRunEvent: (event) => {
    for (const client of uiNoatinWorkEventClients) {
      try {
        writeSseEvent(client, {
          type: 'noatinwork:scheduled-task-ran',
          properties: {
        ...
        uiNoatinWorkEventClients.delete(client);
      }
    }
  },
  logger: console,
...
const emitSessionCreatedEvent = (event) => {
  for (const client of uiNoatinWorkEventClients) {
    try {
      writeSseEvent(client, {
        type: 'noatinwork:session-created',
        properties: {
...
const noatinWorkSessionService = createNoatinWorkSessionService({
  readSettingsFromDiskMigrated,
  sanitizeProjects,
...
const noatinWorkControlService = createNoatinWorkControlService({
  readSettingsFromDiskMigrated,
  sanitizeProjects,
  buildOpenCodeUrl,
  getOpenCodeAuthHeaders,
  waitForOpenCodeReady,
  sessionService: noatinWorkSessionService,
...
async function main(options = {}) {
  ...
  const effectiveBindHost = host
    || (typeof process.env.NOATINWORK_HOST === 'string' && process.env.NOATINWORK_HOST.trim().length > 0
      ? process.env.NOATINWORK_HOST.trim()
      : '127.0.0.1');
  agentToolRuntime = createAgentToolRuntime({
    crypto,
    fsPromises,
    path,
    dataDir: NOATINWORK_DATA_DIR,
    env: process.env,
    executeAction: (...args) => noatinWorkControlService.execute(...args),
    getActivePort: () => {
  ...
  const uiPassword = typeof options.uiPassword === 'string'
    ? options.uiPassword
    : (typeof process.env.NOATINWORK_UI_PASSWORD === 'string' ? process.env.NOATINWORK_UI_PASSWORD : null);
  if (
  ...
  const apiOnly = options.apiOnly === true || isEnvFlagEnabled(process.env.NOATINWORK_API_ONLY);
  ...
  console.log(`Starting NoatinWork on port ${port === 0 ? 'auto' : port}`);
  ...
  const bootstrapResult = bootstrapRuntime.setupBaseRoutes(app, {
    process,
    noatinworkVersion: NOATINWORK_VERSION,
    runtimeName: process.env.NOATINWORK_RUNTIME || 'web',
    serverStartedAt,
    gracefulShutdown,
    getHealthSnapshot: () => {
    ...
    verboseRequestLogs: NOATINWORK_VERBOSE_REQUEST_LOGS,
    uiPassword,
    tunnelAuthController,
    remoteClientAuthRuntime,
    ...
    getServerLabel: () => {
      try {
        ...
        return typeof name === 'string' && name.trim().length > 0 ? name.trim() : 'NoatinWork';
      } catch {
        return 'NoatinWork';
      }
    },
    ...
    server,
    __dirname,
    noatinworkDataDir: NOATINWORK_DATA_DIR,
  ...
  const relayService = createRelayService({
    ...
    hostLock: createRelayHostLock({
      lockFilePath: path.join(NOATINWORK_DATA_DIR, 'relay-host.lock'),
      fs,
      process,
    }),
  ...
  await featureRoutesRuntime.registerRoutes(app, {
    ...
    noatinworkDataDir: NOATINWORK_DATA_DIR,
    noatinworkUserConfigRoot: NOATINWORK_USER_CONFIG_ROOT,
    normalizeDirectoryPath,
    resolveProjectDirectory,
    ...
    scheduledTasksRuntime,
    scheduledTaskService,
    noatinWorkSessionService,
    noatinWorkControlService,
    waitForOpenCodeReady,
    emitSessionCreatedEvent,
    getNoatinWorkEventClients: () => uiNoatinWorkEventClients,
    writeSseEvent,
  ...
  const startupPipelineResult = await startupPipelineRuntime.run({
    ...
    attachSignals,
    apiOnly,
    dictationModelsDir: path.join(NOATINWORK_USER_CONFIG_ROOT, 'speech-models'),
  });
  ...
  // Relay demand can change outside our routes: `noatinwork connect-url

...
export const registerNoatinWorkControlRoutes = (app, { controlService }) => {
  app.post('/api/noatinwork/control', express.json({ limit: '1mb' }), async (req, res) => {
    ...
    try {
      ...
      const controlError = asControlError(error, 'NoatinWork control action failed');

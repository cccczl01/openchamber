...
const SYNC_CONTEXT_GLOBAL_KEY = "__noatinwork_sync_context__"
...
const handleUiNotificationEvent = (payload: Event, fallbackDirectory: string): boolean => {
  if ((payload as { type?: unknown }).type !== "noatinwork:notification") {
    return false
  }
...
const dispatchVSCodeRuntimeNotificationEvent = (directory: string, payload: Event) => {
  ...
  window.dispatchEvent(new CustomEvent("noatinwork:vscode-notification-event", {
...
export function handleEvent(
  ...
  if ((payload as { type?: unknown }).type === "noatinwork:permission-auto-accept.updated") {
...
const dispatchOpenCodeUpdateAvailable = (payload: { version: string }) => {
  ...
  window.dispatchEvent(new CustomEvent("noatinwork:opencode-update-available", { detail: payload }))
}
...
export function SyncProvider(props: {
  ...
  useEffect(() => {
    ...
    window.addEventListener("noatinwork:system-resume", onSystemResume)
    return () => window.removeEventListener("noatinwork:system-resume", onSystemResume)
  }, [childStores])

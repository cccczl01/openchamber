...
async function resolveTargetPort(options = {}, deps = {}) {
  ...
  if (ports.length > 1) {
    ...
    throw new TunnelCliError(
      `Multiple NoatinWork instances are running (ports: ${ports.join(', ')}). Choose one with --port <port>.`,
  ...
  throw new TunnelCliError(
    'No running NoatinWork server found. Start one with `noatinwork serve`, or pass --port <port>.',

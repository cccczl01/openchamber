...
import {
  NOATINWORK_AGENT_TOOL_ACTION_DEFINITIONS,
  NOATINWORK_AGENT_TOOL_ACTIONS,
} from '../noatinwork-control/actions.js';

const TOOL_SCHEMA_VERSION = 1;
const ACTIONS = new Set(NOATINWORK_AGENT_TOOL_ACTIONS);
const AGENT_TOOL_ACTION_TITLES = Object.fromEntries(
  NOATINWORK_AGENT_TOOL_ACTION_DEFINITIONS.map(({ action, title }) => [action, title]),
);
...
export const NoatinWorkPlugin = async () => ({
  tool: {
    noatinwork: {
      description: "Control NoatinWork projects, sessions, and scheduled tasks on the user's behalf. Sessions and scheduled tasks you create are for the user to follow and interact with; never use this tool to delegate parts of your own current task. Use one action per call. Scope with projectId or directory; omit both to use the current session directory. Session dispatches return immediately by default and you receive no notification when a dispatched session finishes, so never promise to report back on it; the user follows it in NoatinWork; a dispatched session needs no follow-up from you. If the user later asks how it went, use session.messages (add wait to block until it is idle, lastAssistant for just the final answer) — session.send always sends a NEW prompt and never just waits. Set wait only when the user asks or the next step requires the completed result. Session and worktree deletion are unavailable.",
      args: {
        action: { type: "string", enum: ${JSON.stringify(NOATINWORK_AGENT_TOOL_ACTIONS)}, oneOf: ${JSON.stringify(NOATINWORK_AGENT_TOOL_ACTION_DEFINITIONS.map(({ action, description }) => ({ const: action, description })))}, description: "NoatinWork action to perform" },
      ...
      async execute(input, context) {
        ...
        const endpoint = process.env.NOATINWORK_AGENT_TOOL_URL
        const token = process.env.NOATINWORK_AGENT_TOOL_TOKEN
        const failure = (payload) => ({
          title,
          output: JSON.stringify(payload),
          metadata: { noatinwork: { schemaVersion: ${TOOL_SCHEMA_VERSION}, action: args.action, description: title, ok: false } },
        })
        if (!endpoint || !token) {
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "NoatinWork managed tool connection is unavailable" } })
        }
        ...
        try {
          ...
          if (valid) return { title, output, metadata: { noatinwork: { schemaVersion: ${TOOL_SCHEMA_VERSION}, action: args.action, description: title, ok: result.ok === true } } }
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "NoatinWork returned an invalid response", kind: "runtime", status: response.status } })
        } catch (error) {
...
const mergePluginConfig = (rawConfig, pluginUrl) => {
  ...
  if (errors.length > 0 || !parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('OPENCODE_CONFIG_CONTENT must contain a valid JSON object before NoatinWork can inject its managed tool');
  }
  if (parsed.plugin !== undefined && !Array.isArray(parsed.plugin)) {
    throw new Error('OPENCODE_CONFIG_CONTENT plugin must be an array before NoatinWork can inject its managed tool');
  }
...
export const createAgentToolRuntime = (dependencies) => {
  ...
  const pluginPath = path.join(pluginDirectory, 'noatinwork-plugin.js');
  let activeToken = null;

  const prepareManagedOpenCodeEnv = async () => {
    const port = getActivePort();
    if (!Number.isInteger(port) || port <= 0) {
      throw new Error('NoatinWork listener port is unavailable for managed tool injection');
    }
    ...
    return {
      ...
      NOATINWORK_AGENT_TOOL_URL: `http://127.0.0.1:${port}/api/noatinwork/agent-tool`,
      NOATINWORK_AGENT_TOOL_TOKEN: activeToken,
    };
  };
  ...
  const execute = async (payload = {}, options = {}) => {
    ...
    if (!action || !ACTIONS.has(action)) {
      return createResult({ ok: false, action, error: { message: `Unsupported NoatinWork action: ${action || 'missing'}`, kind: 'usage' } });
    }
    if (typeof executeAction !== 'function') {
      return createResult({ ok: false, action, error: { message: 'NoatinWork control service is unavailable', kind: 'runtime' } });
    }
    try {
  ...
  const registerRoutes = (app, express) => {
    app.post('/api/noatinwork/agent-tool', express.json({ limit: '1mb' }), async (req, res) => {

...
const MINI_CHAT_PRESENCE_CHANNEL = 'noatinwork:mini-chat-presence';
...
const MiniChatBootstrap: React.FC<{ config: MiniChatConfig }> = ({ config }) => {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('noatinwork:open-session', onOpenSession);
    return () => window.removeEventListener('noatinwork:open-session', onOpenSession);

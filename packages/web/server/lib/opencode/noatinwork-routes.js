...
function resolveSystemdServiceUnit(environment) {
  ...
  const configuredUnit = typeof environment.NOATINWORK_SYSTEMD_UNIT === 'string'
    ? environment.NOATINWORK_SYSTEMD_UNIT.trim()
    : '';
  const unit = configuredUnit || 'noatinwork.service';
...
export const registerNoatinWorkRoutes = (app, dependencies) => {
  const {
    fs,
    path,
    process,
  ...
  app.get('/api/noatinwork/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/noatinwork/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(noatinworkDataDir, 'run', `noatinwork-${currentPort}.json`);
      ...
      if (isForegroundService) {
        if (!systemdServiceUnit) {
          return res.status(409).json({
            error: 'Foreground servers must be updated by their service manager. Set NOATINWORK_SYSTEMD_UNIT when running under systemd, or run noatinwork update and restart the service.',
          });
        }

        const updateJobName = `noatinwork-update-${Date.now()}`;
        ...
        return res.json({
          success: true,
          message: 'Update queued; NoatinWork will restart after installation completes',
          version: updateInfo.version,
          packageManager: pm,
          autoRestart: true,
      ...
      let restartCmdFallback = `noatinwork serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(noatinworkDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== NoatinWork update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting NoatinWork...
              ${restartCmd || 'echo Service manager will restart NoatinWork.'}
            ) else (
              echo Update failed
              exit /b 1
            )
  ...
  app.get('/api/noatinwork/models-metadata', async (_req, res) => {
    try {

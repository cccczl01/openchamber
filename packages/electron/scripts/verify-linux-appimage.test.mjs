...
const createPayload = () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'noatinwork-payload-test-'));
  fs.writeFileSync(path.join(root, 'noatinwork.desktop'), [
    '[Desktop Entry]', 'Name=NoatinWork', 'Exec=AppRun --no-sandbox %U', 'Icon=noatinwork', 'StartupWMClass=noatinwork', '',
  ].join('\n'));
  writeElf(path.join(root, 'noatinwork'), 'x64');
...
test('reads supported ELF architectures', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'noatinwork-elf-test-'));
  try {

...
const getLocalRuntimeOrigin = (): string => {
  ...
  const value = (window as typeof window & { __NOATINWORK_LOCAL_ORIGIN__?: string }).__NOATINWORK_LOCAL_ORIGIN__;
...
const createVSCodeWorkspaceProjects = (
  ...
  if (streamDebugEnabled()) {
    console.log('[NoatinWork][VSCode][projects] Using workspace projects', projects);
  }
...
if (typeof window !== 'undefined') {
  window.addEventListener('noatinwork:settings-synced', (event: Event) => {

...
Server-owned scheduled task runtime and routes for NoatinWork-only automation.
...
- This module is NoatinWork feature logic; it is intentionally separate from OpenCode proxy/runtime internals.
...
- `packages/web/server/lib/scheduled-tasks/runtime.js`
  ...
  - Emits NoatinWork task-run events
...
- `packages/web/server/lib/scheduled-tasks/routes.js`
  ...
  - Manual run endpoint
  - NoatinWork events SSE stream endpoint
...
- Registers:
  ...
  - `GET /api/noatinwork/scheduled-tasks/status`
  - `GET /api/noatinwork/events`

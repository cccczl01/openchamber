...
function resolveConfiguredBindHost(hostOverride) {
  const configured = typeof hostOverride === 'string' && hostOverride.trim()
    ? hostOverride.trim()
    : typeof process.env.NOATINWORK_HOST === 'string'
...
function formatUnsafePortWarning(port) {
  return `Port ${port} is browser-unsafe (ERR_UNSAFE_PORT) and is not supported for NoatinWork UI at ${buildLocalUrl(port, '/')}.`;
}

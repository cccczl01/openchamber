...
type NoatinWorkOpencodeResolution = {
  configured?: unknown;
  resolved?: unknown;
  resolvedDir?: unknown;
  source?: unknown;
...
const buildOpenCodeStatusReport = async (): Promise<string> => {
  ...
  const noatinWorkHealth: NoatinWorkHealthSnapshot | null = await (async () => {
    ...
    try {
      ...
      return json as NoatinWorkHealthSnapshot;
    } catch {
      return null;
    } finally {
      clearTimeout(timeout);
  ...
  const noatinWorkOpencodeResolutionResult: {
    data: NoatinWorkOpencodeResolution | null;
    status: number | null;
    error: string | null;
  } = await (async () => {
    ...
    try {
      ...
      return { data: json as NoatinWorkOpencodeResolution, status: resp.status, error: null };
    } catch (error) {
      return {
        data: null,
        status: null,
  ...
  lines.push(`NoatinWork version: ${appVersion}`);
  ...
  const runtimeOpenCodePort = normalizePort(noatinWorkHealth?.openCodePort);
  lines.push(`OpenCode runtime port: ${runtimeOpenCodePort ?? '(unknown)'}`);
  if (typeof noatinWorkHealth?.openCodeRunning === 'boolean') {
    lines.push(`OpenCode runtime running: ${noatinWorkHealth.openCodeRunning ? 'yes' : 'no'}`);
  }
  if (typeof noatinWorkHealth?.openCodeSecureConnection === 'boolean') {
    lines.push(`Secure OpenCode connection: ${noatinWorkHealth.openCodeSecureConnection ? 'true' : 'false'}`);
  }
  if (typeof noatinWorkHealth?.openCodeAuthSource === 'string' && noatinWorkHealth.openCodeAuthSource.trim()) {
    lines.push(`OpenCode auth source: ${noatinWorkHealth.openCodeAuthSource}`);
  }

  if (typeof window !== 'undefined') {
    const injected = (window as unknown as { __NOATINWORK_MACOS_MAJOR__?: unknown }).__NOATINWORK_MACOS_MAJOR__;
  ...
  if (isLikelyMac) {
    ...
    const launchDiagnostics = isRecord(noatinWorkHealth?.lastOpenCodeLaunchDiagnostics)
      ? noatinWorkHealth.lastOpenCodeLaunchDiagnostics
      : null;
    const actualLaunchArgs = launchDiagnostics && Array.isArray(launchDiagnostics.args)
      ...
      : [];
    const noatinWorkOpencodeResolution = noatinWorkOpencodeResolutionResult.data;
    const configured =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.configured === 'string'
        ? noatinWorkOpencodeResolution.configured
        : null;
    const resolved =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.resolved === 'string'
        ? noatinWorkOpencodeResolution.resolved
        : (noatinWorkHealth && typeof noatinWorkHealth.opencodeBinaryResolved === 'string' ? noatinWorkHealth.opencodeBinaryResolved : '');
    const resolvedDir =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.resolvedDir === 'string'
        ? noatinWorkOpencodeResolution.resolvedDir
        : '';
    const source =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.source === 'string'
        ? noatinWorkOpencodeResolution.source
        : (noatinWorkHealth && typeof noatinWorkHealth.opencodeBinarySource === 'string' ? noatinWorkHealth.opencodeBinarySource : '');
    const configuredLaunchBinary =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.launchBinary === 'string'
        ? noatinWorkOpencodeResolution.launchBinary
        : (noatinWorkHealth && typeof noatinWorkHealth.opencodeLaunchBinary === 'string' ? noatinWorkHealth.opencodeLaunchBinary : '');
    const configuredLaunchWrapperType =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.launchWrapperType === 'string'
        ? noatinWorkOpencodeResolution.launchWrapperType
        : (noatinWorkHealth && typeof noatinWorkHealth.opencodeLaunchWrapperType === 'string' ? noatinWorkHealth.opencodeLaunchWrapperType : '');
    const configuredLaunchArgs =
      noatinWorkOpencodeResolution && Array.isArray(noatinWorkOpencodeResolution.launchArgs)
        ? noatinWorkOpencodeResolution.launchArgs.filter((value): value is string => typeof value === 'string')
        : (noatinWorkHealth && Array.isArray(noatinWorkHealth.opencodeLaunchArgs)
          ? noatinWorkHealth.opencodeLaunchArgs.filter((value): value is string => typeof value === 'string')
          : []);
    const node =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.node === 'string'
        ? noatinWorkOpencodeResolution.node
        : (noatinWorkHealth && typeof noatinWorkHealth.nodeBinaryResolved === 'string' ? noatinWorkHealth.nodeBinaryResolved : '');
    const bun =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.bun === 'string'
        ? noatinWorkOpencodeResolution.bun
        : (noatinWorkHealth && typeof noatinWorkHealth.bunBinaryResolved === 'string' ? noatinWorkHealth.bunBinaryResolved : '');
    const detectedNow =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.detectedNow === 'string'
        ? noatinWorkOpencodeResolution.detectedNow
        : '';
    const detectedSourceNow =
      noatinWorkOpencodeResolution && typeof noatinWorkOpencodeResolution.detectedSourceNow === 'string'
        ? noatinWorkOpencodeResolution.detectedSourceNow
        : '';
    ...
    if (!noatinWorkOpencodeResolution && noatinWorkOpencodeResolutionResult.error) {
      lines.push(`- resolution-endpoint: ${noatinWorkOpencodeResolutionResult.error}`);
    }
  }

...
// foreign platform's chrome, not NoatinWork's own status tokens, so the
...
export const WindowsWindowControls = React.memo(function WindowsWindowControls({
  ...
  useEffect(() => {
    ...
    window.addEventListener('noatinwork:window-maximized-changed', handleMaximizedChange);
    return () => {
      disposed = true;
      window.removeEventListener('noatinwork:window-maximized-changed', handleMaximizedChange);
    };
  }, [visible]);

...
# alias; NOATINWORK_UI_PASSWORD is the canonical runtime variable.
if [ -z "${NOATINWORK_UI_PASSWORD:-}" ] && [ -n "${UI_PASSWORD:-}" ]; then
  NOATINWORK_UI_PASSWORD="$UI_PASSWORD"
  export NOATINWORK_UI_PASSWORD
fi

if [ -n "${NOATINWORK_UI_PASSWORD:-}" ]; then
...
NOATINWORK_HOST="${NOATINWORK_HOST:-0.0.0.0}"
export NOATINWORK_HOST
...
if [ -n "${NOATINWORK_UI_PASSWORD:-}" ]; then
  set -- "$@" --ui-password "$NOATINWORK_UI_PASSWORD"
fi
"$@"

import { getRuntimeExtraHeadersSync, refreshLocalRuntimeUrlAuthToken, refreshRuntimeUrlAuthToken, setRuntimeBearerToken, setRuntimeExtraHeaders } from '@noatinwork-app/ui/lib/runtime-auth';
import { installRuntimeFetchBridge } from '@noatinwork-app/ui/lib/runtime-fetch';
...
import { configureRuntimeUrlResolver } from '@noatinwork-app/ui/lib/runtime-url';
import type { EmbeddedSessionRuntimeBootstrap } from '@noatinwork-app/ui/components/layout/contextPanelEmbeddedChat';
...
declare global {
  interface Window {
    __NOATINWORK_API_BASE_URL__?: string;
    __NOATINWORK_CLIENT_TOKEN__?: string;
    __NOATINWORK_RUNTIME_HEADERS__?: Record<string, string>;
    __NOATINWORK_LOCAL_ORIGIN__?: string;
    __NOATINWORK_RELAY_HOST_ID__?: string;
  }
}
...
export const readRuntimeBootstrapConfig = (): EmbeddedSessionRuntimeBootstrap => {
  ...
  return {
    apiBaseUrl: readString(window.__NOATINWORK_API_BASE_URL__),
    clientToken: readString(window.__NOATINWORK_CLIENT_TOKEN__),
    localOrigin: readString(window.__NOATINWORK_LOCAL_ORIGIN__),
    runtimeHeaders: window.__NOATINWORK_RUNTIME_HEADERS__,
    relayHostId: readString(window.__NOATINWORK_RELAY_HOST_ID__),
  };
};

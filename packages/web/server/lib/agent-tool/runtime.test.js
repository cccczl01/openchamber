...
import { NOATINWORK_AGENT_TOOL_ACTION_DEFINITIONS, NOATINWORK_CONTROL_ACTION_DEFINITIONS } from '../noatinwork-control/actions.js';
...
const createRuntime = async (overrides = {}) => {
  const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'noatinwork-agent-tool-'));
...
describe('agent tool action allowlist', () => {
  it('defines a short title and agent description for every action', () => {
    expect(NOATINWORK_CONTROL_ACTION_DEFINITIONS.every(({ action, title, description }) => action && title && description)).toBe(true);
  });
...
describe('managed agent tool runtime', () => {
  it('materializes the plugin and preserves configured plugin entries', async () => {
    ...
    const pluginPath = path.join(dataDir, 'agent-tool', 'noatinwork-plugin.js');
    ...
    expect(config.plugin).toEqual([
      ...
      expect.stringContaining('/agent-tool/noatinwork-plugin.js'),
    ]);
    expect(preparedEnv.NOATINWORK_AGENT_TOOL_URL).toBe('http://127.0.0.1:3901/api/noatinwork/agent-tool');
    expect(preparedEnv.NOATINWORK_AGENT_TOOL_TOKEN).toMatch(/^[A-Za-z0-9_-]+$/);
    expect(source).toContain('noatinwork: {');
    for (const { action, description } of NOATINWORK_AGENT_TOOL_ACTION_DEFINITIONS) {
    ...
    const hooks = await pluginModule.NoatinWorkPlugin();
    expect(hooks.tool.noatinwork.description).toContain('Session dispatches return immediately by default');
    expect(hooks.tool.noatinwork.description).toContain('Set wait only when the user asks or the next step requires the completed result');
    expect(hooks.tool.noatinwork.args.action.oneOf).toContainEqual({
      const: 'session.messages',
      description: 'Read text-only messages and current sessionStatus for sessionId; directory and limit 10 are defaults',
    });
    expect(hooks.tool.noatinwork.args.parameters.properties.wait.description).toBe(
    ...
    expect(hooks.tool.noatinwork.args.parameters.properties.sessionId).toEqual({ type: 'string' });
    expect(source).not.toContain('title: "NoatinWork"');
    expect(source).not.toContain('@opencode-ai/plugin');
    expect(source).not.toContain(preparedEnv.NOATINWORK_AGENT_TOOL_TOKEN);
  });
  ...
  it('forwards cancellation to the shared control service', async () => {
    const executeAction = vi.fn(async (_action, _input, _directory, options) => {
      await new Promise((resolve, reject) => {
        options.signal.addEventListener('abort', () => reject(Object.assign(new Error('NoatinWork action was cancelled'), { statusCode: 499 })), { once: true });
      });
    });
    const { runtime } = await createRuntime({ executeAction });
    ...
    await expect(pending).resolves.toEqual(expect.objectContaining({
      ok: false,
      action: 'projects.list',
      error: { message: 'NoatinWork action was cancelled', kind: 'runtime' },
    }));
  ...
  it('requires the per-child token on the loopback route', async () => {
    ...
    await request(app)
      .post('/api/noatinwork/agent-tool')
      .send({ input: { action: 'projects.list' } })
      .expect(401);

    const response = await request(app)
      .post('/api/noatinwork/agent-tool')
      .set('authorization', `Bearer ${env.NOATINWORK_AGENT_TOOL_TOKEN}`)
      .send({ input: { action: 'projects.list' } })
      .expect(200);
  ...
  it('executes through the materialized plugin and authenticated callback', async () => {
    ...
    const previousUrl = process.env.NOATINWORK_AGENT_TOOL_URL;
    const previousToken = process.env.NOATINWORK_AGENT_TOOL_TOKEN;
    try {
      const env = await runtime.prepareManagedOpenCodeEnv();
      process.env.NOATINWORK_AGENT_TOOL_URL = env.NOATINWORK_AGENT_TOOL_URL;
      process.env.NOATINWORK_AGENT_TOOL_TOKEN = env.NOATINWORK_AGENT_TOOL_TOKEN;
      const pluginPath = path.join(dataDir, 'agent-tool', 'noatinwork-plugin.js');
      const pluginModule = await import(`${pathToFileURL(pluginPath).href}?test=${Date.now()}`);
      const hooks = await pluginModule.NoatinWorkPlugin();
      const metadata = vi.fn();

      const result = await hooks.tool.noatinwork.execute(
      ...
      expect(result.metadata.noatinwork.description).toBe('List configured projects');
      expect(metadata).toHaveBeenCalledWith(expect.objectContaining({
        title: 'List configured projects',
        metadata: expect.objectContaining({
          noatinwork: expect.objectContaining({ description: 'List configured projects' }),
        }),
      }));
    } finally {
      if (previousUrl === undefined) delete process.env.NOATINWORK_AGENT_TOOL_URL;
      else process.env.NOATINWORK_AGENT_TOOL_URL = previousUrl;
      if (previousToken === undefined) delete process.env.NOATINWORK_AGENT_TOOL_TOKEN;
      else process.env.NOATINWORK_AGENT_TOOL_TOKEN = previousToken;

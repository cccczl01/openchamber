...
const TRAY_ACTION_EVENT = 'noatinwork:tray-action';
...
type TraySnapshot = {
  ...
  // Active instance label (e.g. "Local NoatinWork" or a remote host name) so
...
// the existing `noatinwork:open-session` / `noatinwork:open-draft-session`
...
const isTrayPlatform = (): boolean => {
  ...
  const platform = (window as unknown as { __NOATINWORK_PLATFORM__?: string }).__NOATINWORK_PLATFORM__;
  return platform === 'darwin' || platform === 'win32' || platform === 'linux';
};

const isTrayEnabled = (): boolean =>
  typeof window !== 'undefined' && window.__NOATINWORK_ELECTRON__?.trayEnabled !== false;
...
// the local origin shows as "Local NoatinWork"; a remote host shows its
// configured name. Async because the host config is read over IPC.
const resolveInstanceName = async (): Promise<string> => {
  try {
    if (isDesktopLocalOriginActive()) return 'Local NoatinWork';
    const localOrigin = (window as unknown as { __NOATINWORK_LOCAL_ORIGIN__?: string }).__NOATINWORK_LOCAL_ORIGIN__
      || window.location.origin;
    const runtimeApiBaseUrl = getRuntimeApiBaseUrl();
    if (runtimeApiBaseUrl && locationMatchesHost(runtimeApiBaseUrl, localOrigin)) return 'Local NoatinWork';
...
export const useTraySync = (): void => {
  ...
  React.useEffect(() => {
    ...
    const bridge = (window as unknown as { __NOATINWORK_DESKTOP__?: DesktopBridgeGlobal }).__NOATINWORK_DESKTOP__;

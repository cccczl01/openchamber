...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/NoatinWork route wiring.
...
- `packages/web/server/lib/opencode/noatinwork-routes.js`: NoatinWork update and models metadata route registration.
...
NoatinWork tool injection. Managed launch env strips AppImage `ARGV0` before
...
- `registerSettingsUtilityRoutes(app, dependencies)`: registers small settings utility endpoints:
  ...
  - `POST /api/config/reload` — applies accumulated deferred OpenCode config changes. Managed OpenCode restarts and returns `requiresReload: true`. External OpenCode returns `requiresManualRestart: true` (changes are already on disk; the connected server must be restarted outside NoatinWork).
...
The pipeline binds the NoatinWork listener and publishes its active port
before starting managed OpenCode. The managed custom tool therefore receives
an authoritative loopback callback URL even when NoatinWork binds port `0`.

## Public exports (noatinwork-routes.js)
- `registerNoatinWorkRoutes(app, dependencies)`: registers NoatinWork endpoints:
  - `GET /api/noatinwork/update-check`
  - `POST /api/noatinwork/update-install`
    - Foreground servers running under a systemd user unit queue installation in
      ...
      `NOATINWORK_SYSTEMD_UNIT` overrides the default `noatinwork.service`.
  - `GET /api/noatinwork/models-metadata`
  - `GET /api/zen/models`
...
- Rate limit config: `NOATINWORK_RATE_LIMIT_MAX_ATTEMPTS`, `NOATINWORK_RATE_LIMIT_NO_IP_MAX_ATTEMPTS` env vars.

...
const markSessionAsNoatinWorkCreatedCalls: string[] = []
...
mock.module("../session-ui-store", () => ({
  useSessionUIStore: {
    getState: () => ({
      ...
      markSessionAsNoatinWorkCreated: (sessionId: string) => {
        markSessionAsNoatinWorkCreatedCalls.push(sessionId)
      },
    }),
  },
}))
...
beforeEach(() => {
  ...
  markSessionAsNoatinWorkCreatedCalls.length = 0

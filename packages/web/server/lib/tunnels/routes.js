export const createTunnelRoutesRuntime = (dependencies) => {
  ...
  const registerRoutes = (app) => {
    app.get('/api/noatinwork/tunnel/check', async (req, res) => {
      try {
    ...
    app.post('/api/noatinwork/tunnel/doctor', handleTunnelDoctor);
    app.get('/api/noatinwork/tunnel/doctor', handleTunnelDoctor);

    app.get('/api/noatinwork/tunnel/providers', (_req, res) => {
      const providers = tunnelProviderRegistry.listCapabilities();
      return res.json({ providers });
    });

    app.get('/api/noatinwork/tunnel/status', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.put('/api/noatinwork/tunnel/managed-remote-token', async (req, res) => {
      try {
    ...
    app.post('/api/noatinwork/tunnel/start', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.post('/api/noatinwork/tunnel/stop', (_req, res) => {
      let revokedBootstrapCount = 0;
      let invalidatedSessionCount = 0;
      const activeTunnelId = tunnelAuthController.getActiveTunnelId();

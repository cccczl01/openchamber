...
const previousDataDir = process.env.NOATINWORK_DATA_DIR;
const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'noatinwork-quota-store-'));
process.env.NOATINWORK_DATA_DIR = temporaryDirectory;
...
afterAll(() => {
  if (previousDataDir === undefined) delete process.env.NOATINWORK_DATA_DIR;
  else process.env.NOATINWORK_DATA_DIR = previousDataDir;

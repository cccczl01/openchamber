...
const GITHUB_BACKEND_DISABLED_ERROR = 'NoatinWork VS Code backend GitHub integration is disabled. Use native VS Code GitHub integrations.';
    ...
    const permissionAutoAcceptResponse = await handlePermissionAutoAcceptBridgeMessage(
      ...
      ctx?.context,
      {
        broadcast: (snapshot) => vscode.commands.executeCommand(
          'noatinwork.internal.permissionAutoAcceptSynced',
          snapshot,
        ),
      },
    );

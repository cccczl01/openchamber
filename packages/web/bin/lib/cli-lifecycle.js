...
import {
  ...
  readInstanceOptions,
  removeInstanceFile,
  getNoatinWorkProcessState,
  hasNoatinWorkRuntimeInfo,
} from './cli-process.js';
...
function createLivePortInstance(port, info, host) {
  if (!hasNoatinWorkRuntimeInfo(info)) return null;
  return {
    port,
    pid: Number.isFinite(info.pid) ? info.pid : null,
    pidFilePath: path.join(getRunDir(), `noatinwork-${port}.pid`),
    instanceFilePath: path.join(getRunDir(), `noatinwork-${port}.json`),
    mtime: 0,
    startedAt: 0,
    launchMode: 'daemon',
...
async function fetchSystemInfoFromPortCandidates(port, fetchImpl, hosts, expectedPid) {
  for (const { host, requiresPidMatch } of hosts) {
    ...
    if (hasNoatinWorkRuntimeInfo(info)) {
...
async function resolveDoctorPortStatuses(options = {}) {
  ...
  if (options.explicitPort) {
    ...
    if (desktopEntry && desktopEntry.port === requestedPort) {
      statuses.push({
        ...
        detail: 'Use a CLI instance port from `noatinwork serve` for tunneling.',
      });
    ...
    statuses.push({
      ...
      detail: `Start one with \`noatinwork serve --port ${requestedPort}\`.`,
    });
    return { statuses, availableEntries: [] };
  }
  ...
  if (runningEntries.length === 0) {
    statuses.push({
      ...
      detail: 'Start one with `noatinwork serve`.',
    });
  }
...
async function discoverRunningInstances(options = {}) {
  ...
  const getProcessState = typeof options.getNoatinWorkProcessState === 'function'
    ? options.getNoatinWorkProcessState
    : (pid) => getNoatinWorkProcessState(pid, options);
  try {
    const files = fs.readdirSync(runDir);
    const pidFiles = files.filter((file) => file.startsWith('noatinwork-') && file.endsWith('.pid'));
    for (const file of pidFiles) {
      const port = parseInt(file.replace('noatinwork-', '').replace('.pid', ''), 10);
      if (!Number.isFinite(port) || port <= 0) continue;
      const pidFilePath = path.join(runDir, file);
      const pid = readPidFile(pidFilePath);
      if (!pid) {
        removePidFile(pidFilePath);
        removeInstanceFile(path.join(runDir, `noatinwork-${port}.json`));
        continue;
      }

      const instanceFilePath = path.join(runDir, `noatinwork-${port}.json`);
      ...
      // confirms NoatinWork. Cmdline identity alone can match a recycled PID
      // from another NoatinWork process on a different port. Try all plausible
      ...
      if (!hasNoatinWorkRuntimeInfo(liveInfo)) {
...
async function discoverNoatinWorkInstanceOnPort(port, options = {}) {
...
async function discoverLifecycleInstances(options = {}, deps = {}) {
  ...
  const liveInstance = await discoverNoatinWorkInstanceOnPort(options.port, {
    ...deps,
    host: options.host,
    runningInstances,
  });
...
async function discoverUnconfirmedRegistryInstanceOnPort(port, options = {}) {
  ...
  const storedOptions = readInstanceOptions(instanceFilePath);
  const processState = getNoatinWorkProcessState(pid);
  if (processState === 'dead') {
    removePidFile(pidFilePath);
    removeInstanceFile(instanceFilePath);
    return null;

        ...
        try {
          ...
          if (typeof process.send === 'function') {
            if (!process.connected) {
              throw new Error('NoatinWork startup IPC channel disconnected before ready notification');
            }

            await new Promise((resolveReadyNotification, rejectReadyNotification) => {
              try {
                process.send({ type: 'noatinwork:ready', port: activePort }, (error) => {
                  if (error) {
          ...
          console.log(`NoatinWork server listening on ${bindHost}:${activePort}`);

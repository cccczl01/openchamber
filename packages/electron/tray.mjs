...
//
// Surfaces a glanceable, always-visible view of NoatinWork's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'NoatinWork — no active sessions';
  const bits = [];
  ...
  return `NoatinWork — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: 'Show NoatinWork', click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: 'Quit NoatinWork', click: () => onAction({ type: 'quit' }) },
      );
    }

...
import type { NoatinWorkSection } from '@/components/sections/noatinwork/types';
import { NoatinWorkPage } from '@/components/sections/noatinwork/NoatinWorkPage';
import { AboutSettings } from '@/components/sections/noatinwork/AboutSettings';
...
const SETTINGS_DETAIL_HISTORY_KEY = '__noatinworkSettingsDetail';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed, visiblePageSlugs, initialMobileStage = 'nav' }) => {
  ...
  const isWindows = React.useMemo(() => {
    return isDesktopShell() && typeof window !== 'undefined'
      && (window as unknown as { __NOATINWORK_PLATFORM__?: string }).__NOATINWORK_PLATFORM__ === 'win32';
  }, []);
  const isLinux = React.useMemo(() => {
    return isDesktopShell() && typeof window !== 'undefined'
      && (window as unknown as { __NOATINWORK_PLATFORM__?: string }).__NOATINWORK_PLATFORM__ === 'linux';
  }, []);
  ...
  const noatinWorkSectionBySlug: Partial<Record<SettingsPageSlug, NoatinWorkSection>> = React.useMemo(() => ({
    general: 'general',
    appearance: 'visual',
    chat: 'chat',
    shortcuts: 'shortcuts',
  ...
  const openSearchResult = React.useCallback((result: SettingsSearchResult) => {
    ...
    if (result.id === 'plugins.create' && typeof window !== 'undefined') {
      window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('noatinwork:settings-open-plugin-add'));
      }, 50);
    }
  ...
  const renderPageContent = React.useCallback((slug: SettingsPageSlug) => {
    ...
    switch (slug) {
      ...
      case 'voice':
      case 'tunnel': {
        const section = noatinWorkSectionBySlug[slug] ?? 'visual';
        return <NoatinWorkPage section={section} />;
      }
      case 'home':
      default:
        return null;
    }
  }, [noatinWorkSectionBySlug, renderUnavailable, runtimeCtx, t]);

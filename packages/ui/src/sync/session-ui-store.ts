...
import { getWorktreeSetupWaitEnabled } from "@/lib/noatinworkConfig"
...
export type SessionUIState = {
  ...
  clearError: () => void
  markSessionAsNoatinWorkCreated: (sessionId: string) => void
  isNoatinWorkCreatedSession: (sessionId: string) => boolean
  getContextUsage: (contextLimit: number, outputLimit: number) => SessionContextUsage | null
  initializeNewNoatinWorkSession: (sessionId: string, agents: unknown[]) => void
...
export async function materializeOpenDraftSession(selection: {
  ...
  store.initializeNewNoatinWorkSession(created.id, configState.agents ?? [])
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  markSessionAsNoatinWorkCreated: (sessionId) =>
    set((s) => {
  ...
  isNoatinWorkCreatedSession: (sessionId) => get().webUICreatedSessions.has(sessionId),
  ...
  initializeNewNoatinWorkSession: () => {

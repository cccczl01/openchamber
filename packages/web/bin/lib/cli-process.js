...
async function getPidFilePath(port) {
  return path.join(getRunDir(), `noatinwork-${port}.pid`);
}

async function getInstanceFilePath(port) {
  return path.join(getRunDir(), `noatinwork-${port}.json`);
}
...
// that PID to an unrelated process — see isNoatinWorkProcessRunning.
...
function isNoatinWorkCmdline(cmdline) {
  if (typeof cmdline !== 'string' || cmdline.length === 0) {
    return false;
  }
  // Every install path contains the "noatinwork" segment — the npm package
  // (@noatinwork-app/web) and the source checkout both do, for the foreground
  ...
  return cmdline.toLowerCase().includes('noatinwork');
}

// Liveness + identity — "is the NoatinWork instance recorded in a pid file
...
function isNoatinWorkProcessRunning(pid) {
  const state = getNoatinWorkProcessState(pid);
  return state === 'matched' || state === 'unknown';
}

function getNoatinWorkProcessState(pid, options = {}) {
  ...
  return isNoatinWorkCmdline(cmdline) ? 'matched' : 'mismatched';
}

function hasNoatinWorkRuntimeInfo(info) {

#!/usr/bin/env bash
# NoatinWork Install Script
# Usage: curl -fsSL https://raw.githubusercontent.com/btriapitsyn/noatinwork/main/scripts/install.sh | bash

set -euo pipefail

PACKAGE_NAME="@noatinwork-app/web"
BIN_NAME="noatinwork"
MIN_NODE_VERSION=22
...
main() {
  ...
  # If NoatinWork is already installed, hand off to its own updater instead
  # of guessing a package manager. `noatinwork update` detects which manager
  ...
  if command_exists "$BIN_NAME"; then
    info "NoatinWork is already installed — updating via 'noatinwork update'..."
    echo ""
    if noatinwork update; then
      echo ""
      success "NoatinWork is up to date!"
      exit 0
    fi
    echo ""
    error "Update failed."
...
EOF
    printf '%b\n' "$NC"
    success "NoatinWork installed successfully!"
    echo ""
    ...
    if command_exists "$BIN_NAME"; then
      echo "  Get started:"
      echo "    noatinwork              # Start server on port 3000"
      echo "    noatinwork --help       # Show all options"
    else
      ...
      if [ -n "$bin_dir" ]; then
        ...
        echo "  then restart your terminal and run: noatinwork"
      fi
    fi
    echo ""
    echo "  Prerequisites:"

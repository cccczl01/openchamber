...
export const applyPersistedHomeDirectoryToWindow = (homeDirectory: string): void => {
  ...
  if (typeof window.__NOATINWORK_HOME__ === 'string' && window.__NOATINWORK_HOME__.length > 0) {
    return;
  }

  try {
    window.__NOATINWORK_HOME__ = homeDirectory;
  } catch {
    /* read-only contextBridge property — leave preload-seeded value */
  }
};

const SETTINGS_MIRROR_INDEX_KEY = 'noatinwork.settingsMirror.v2.index';
const SETTINGS_MIRROR_KEY_PREFIX = 'noatinwork.settingsMirror.v2:';
...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (typeof settings.pwaAppName === 'string') {
    ...
    if (normalized.length > 0) {
      localStorage.setItem('noatinwork.pwaName', normalized);
    } else {
      localStorage.removeItem('noatinwork.pwaName');
    }
  } else {
    localStorage.removeItem('noatinwork.pwaName');
  }
...
const dispatchSettingsSynced = (settings: DesktopSettings): void => {
  ...
  window.dispatchEvent(new CustomEvent<DesktopSettings>('noatinwork:settings-synced', { detail: settings }));
};
...
const dispatchSettingsSaveState = (state: 'saving' | 'saved' | 'error'): void => {
  ...
  window.dispatchEvent(new CustomEvent<'saving' | 'saved' | 'error'>('noatinwork:settings-save-state', { detail: state }));
};
...
export const syncDesktopSettings = async (): Promise<void> => {
  ...
  const applySettings = async (settings: DesktopSettings) => {
    ...
    // `noatinwork:files:auto-save-enabled`. Prefer the hydrated store value and

...
const STARTUP_SERVICE_ID = 'dev.noatinwork.web';
...
function getStartupServicePaths() {
  ...
  if (process.platform === 'linux') {
    return {
      platform: 'linux',
      servicePath: path.join(os.homedir(), '.config', 'systemd', 'user', 'noatinwork.service'),
    };
  }
...
function getMacosStartupWrapperPath() {
  return path.join(getDataDir(), 'bin', 'NoatinWork');
}
...
function collectStartupEnv(options = {}) {
  ...
  if (uiPassword) {
    env.NOATINWORK_UI_PASSWORD = uiPassword;
  }
  if (options.apiOnly === true) {
    env.NOATINWORK_API_ONLY = 'true';
  }
  if (typeof process.env.NOATINWORK_DATA_DIR === 'string' && process.env.NOATINWORK_DATA_DIR.trim().length > 0) {
    env.NOATINWORK_DATA_DIR = path.resolve(process.env.NOATINWORK_DATA_DIR.trim());
  }
  return env;
}
...
function buildMacosLaunchAgent(options = {}) {
  ...
  const env = collectStartupEnv(options);
  const logDir = path.join(os.homedir(), 'Library', 'Logs', 'NoatinWork');
...
function getStartupStatus() {
  ...
  if (paths.platform === 'linux') {
    const enabledResult = runStartupCommand('systemctl', ['--user', 'is-enabled', 'noatinwork.service'], { allowFailure: true });
    const activeResult = runStartupCommand('systemctl', ['--user', 'is-active', 'noatinwork.service'], { allowFailure: true });
...
function enableStartupService(options = {}) {
  ...
  if (paths.platform === 'macos') {
    ...
    fs.mkdirSync(path.join(os.homedir(), 'Library', 'Logs', 'NoatinWork'), { recursive: true, mode: 0o700 });
  ...
  if (paths.platform === 'linux') {
    ...
    runStartupCommand('systemctl', ['--user', 'enable', '--now', 'noatinwork.service']);
    return getStartupStatus();
  }
...
function disableStartupService() {
  ...
  if (paths.platform === 'linux') {
    runStartupCommand('systemctl', ['--user', 'disable', '--now', 'noatinwork.service'], { allowFailure: true });
    try { fs.unlinkSync(paths.servicePath); } catch {}

...
import { opencodeClient } from '@noatinwork-app/ui/lib/opencode/client';
import { sanitizeHeadersForBrowser } from '@noatinwork-app/ui/lib/runtime-fetch';
import {
  buildVSCodeThemeFromPalette,
  readVSCodeThemePalette,
  type VSCodeThemeKind,
  type VSCodeThemePayload,
} from '@noatinwork-app/ui/lib/theme/vscode/adapter';
import { getBootstrapMessages, readStoredLocaleForBootstrap } from '@noatinwork-app/ui/lib/i18n';
import type { VSCodeActiveEditorFile } from '@/sync/input-store';
import { usePermissionStore } from '@noatinwork-app/ui/stores/permissionStore';
import { processVSCodePermissionAutoAccept } from '@noatinwork-app/ui/sync/vscode-permission-auto-accept';
import type { PermissionRequest } from '@opencode-ai/sdk/v2/client';

type ConnectionStatus = 'connecting' | 'connected' | 'error' | 'disconnected';
type PanelType = 'chat' | 'agentManager';

declare const __NOATINWORK_WEBVIEW_BUILD_TIME__: string;
...
declare global {
  interface Window {
    ...
    __NOATINWORK_VSCODE_THEME__?: VSCodeThemePayload['theme'];
    __NOATINWORK_VSCODE_SHIKI_THEMES__?: { light?: Record<string, unknown>; dark?: Record<string, unknown> } | null;
    __NOATINWORK_CONNECTION__?: { status: ConnectionStatus; error?: string; cliAvailable?: boolean };
    __NOATINWORK_HOME__?: string;
    __NOATINWORK_PANEL_TYPE__?: PanelType;
    __NOATINWORK_VSCODE_WINDOW_FOCUSED__?: boolean;
  }
}

console.log('[NoatinWork] VS Code webview starting...');
console.log('[NoatinWork] VS Code webview build:', __NOATINWORK_WEBVIEW_BUILD_TIME__);
console.log('[NoatinWork] Config:', window.__VSCODE_CONFIG__);
try {
  if (window.localStorage.getItem('noatinwork_stream_debug') === '1') {
    console.log('[NoatinWork] Debug: noatinwork_stream_debug=1');
  }
} catch {
  // ignore
}

window.__NOATINWORK_RUNTIME_APIS__ = createVSCodeAPIs();
...
const bootstrapConnectionStatus = () => {
  ...
  window.__NOATINWORK_CONNECTION__ = { status: initialStatus, cliAvailable };
};

bootstrapConnectionStatus();

// Expose panel type globally for the VS Code app root to conditionally render.
window.__NOATINWORK_PANEL_TYPE__ = (window.__VSCODE_CONFIG__?.panelType as PanelType) || 'chat';
...
const handleConnectionMessage = (event: MessageEvent) => {
  ...
  if (msg?.type === 'connectionStatus') {
    ...
    const prevCliAvailable = window.__NOATINWORK_CONNECTION__?.cliAvailable ?? true;
    window.__NOATINWORK_CONNECTION__ = { status: payload, error, cliAvailable: prevCliAvailable };
    window.dispatchEvent(new CustomEvent('noatinwork:connection-status', { detail: { status: payload, error } }));
  }
};

window.addEventListener('message', handleConnectionMessage);
window.addEventListener('noatinwork:connection-status', () => {
...
const maybeHideLoadingOverlay = () => {
  const connectionStatus = window.__NOATINWORK_CONNECTION__?.status ?? 'connecting';
  ...
  if (connectionStatus === 'error') {
    const error = window.__NOATINWORK_CONNECTION__?.error;
...
const emitVSCodeTheme = (preferredKind?: VSCodeThemeKind) => {
  ...
  window.__NOATINWORK_VSCODE_THEME__ = theme;
   applyInitialTheme(theme);
  window.dispatchEvent(new CustomEvent<VSCodeThemePayload>('noatinwork:vscode-theme', {
...
onThemeChange((payload) => {
  ...
  if (typeof payload === 'object' && payload?.shikiThemes !== undefined) {
    window.__NOATINWORK_VSCODE_SHIKI_THEMES__ = payload.shikiThemes;
    window.dispatchEvent(
      new CustomEvent('noatinwork:vscode-shiki-themes', {
...
if (workspaceFolder) {
  ...
  window.__NOATINWORK_HOME__ = normalizedWorkspaceFolder;
  try {
...
const handleLocalApiRequest = async (input: RequestInfo | URL, url: URL, init: RequestInit | undefined, method: string) => {
  ...
  if (normalizedPathname === '/api/system/info' && method === 'GET') {
    ...
    return jsonResponse({
      noatinworkVersion: config?.extensionVersion || 'VS Code Extension',
      runtime: 'vscode',
  ...
  if (normalizedPathname.startsWith('/api/noatinwork/tunnel/')) {
  ...
  // Dictation runs on the NoatinWork web server (WebSocket + worker); the VS
  ...
  if (pathname === '/health' || pathname === '/api/health') {
    const connectionStatus = window.__NOATINWORK_CONNECTION__?.status;
    const isReady = connectionStatus === 'connected';
    const cliAvailable = window.__NOATINWORK_CONNECTION__?.cliAvailable ?? true;
  ...
  if (pathname.startsWith('/api/noatinwork/models-metadata')) {
    try {
      const data = await sendBridgeMessage('api:models/metadata');
      return new Response(JSON.stringify(data), { status: 200, headers: { 'Content-Type': 'application/json' } });
    } catch (error) {
      console.warn('[NoatinWork] Failed to fetch models metadata via bridge, returning empty set:', error);
  ...
  if (pathname.startsWith('/api/noatinwork/update-check')) {
    try {
      ...
      const data = await sendBridgeMessage('api:noatinwork:update-check', {
        currentVersion,
        instanceMode,
        deviceClass,
        platform,
...
window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (targetUrl && targetUrl.hostname.includes('models.dev')) {
    try {
      ...
      console.warn('[NoatinWork] models.dev request failed via bridge, returning empty metadata:', error);
      return new Response(JSON.stringify({}), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
  }
  ...
  Promise.all([
    ...
    if (currentProviderId && currentModelId) {
      ...
      sessionStore.sendMessage(
        ...
        console.error('[NoatinWork] Failed to send prompt:', error);
      });
    } else {
...
onCommand('newSession', (payload) => {
  ...
  window.dispatchEvent(new CustomEvent('noatinwork:navigate', { detail: { view: 'chat' } }));
});
...
onCommand('showSettings', () => {
  ...
  window.dispatchEvent(new CustomEvent('noatinwork:navigate', { detail: { view: 'settings' } }));
});
...
onCommand('reloadOpenCode', () => {
  void import('@noatinwork-app/ui/stores/useAgentsStore').then(({ reloadOpenCodeConfiguration }) => {
...
const claimNoatinWorkNotification = async (payload: { title?: unknown; body?: unknown; sessionId?: unknown; tag?: unknown } | undefined): Promise<boolean> => {
...
const showNoatinWorkNotification = (payload: { title?: unknown; body?: unknown; sessionId?: unknown; tag?: unknown; requireHidden?: unknown } | undefined) => {
  if (typeof Notification === 'undefined') {
    return false;
  }

  const show = async () => {
    const isVSCodeWindowFocused = window.__NOATINWORK_VSCODE_WINDOW_FOCUSED__ ?? document.hasFocus();
    ...
    const sessionId = typeof payload?.sessionId === 'string' && payload.sessionId.trim().length > 0
      ...
      : '';
    if (!await claimNoatinWorkNotification({ ...payload, title, body, sessionId })) {
      return false;
    }
...
onCommand('showNotification', (payload) => {
  showNoatinWorkNotification(payload as { title?: unknown; body?: unknown; sessionId?: unknown; requireHidden?: unknown } | undefined);
});

onCommand('windowFocusChanged', (payload) => {
  if (typeof payload === 'object' && payload && typeof (payload as { focused?: unknown }).focused === 'boolean') {
    window.__NOATINWORK_VSCODE_WINDOW_FOCUSED__ = (payload as { focused: boolean }).focused;
  }
});
  ...
  if (!notificationSettingsSyncPromise) {
    notificationSettingsSyncPromise = import('@/lib/persistence')
      ...
      .catch((error) => {
        ...
        console.warn('[NoatinWork] Failed to sync notification settings:', error);
      });
  }
...
window.addEventListener('noatinwork:vscode-notification-event', (event) => {
...
onCommand('settingsSynced', () => {
  import('@noatinwork-app/ui/lib/persistence').then(({ syncDesktopSettings }) => {
...
import('@noatinwork-app/ui/apps/renderVSCodeApp')
  .then(async ({ renderVSCodeApp }) => {
    renderVSCodeApp(window.__NOATINWORK_RUNTIME_APIS__ ?? createVSCodeAPIs());
    await waitForUiMount();
    uiMounted = true;
    maybeHideLoadingOverlay();
  })
  .catch((error) => {
    console.error('[NoatinWork] Failed to bootstrap UI:', error);

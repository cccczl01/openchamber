...
enum WidgetStore {
    static let appGroup = "group.com.noatinwork.app"
...
enum WidgetDeepLink {
    static func newSession() -> URL { URL(string: "noatinwork://new")! }
    static func attention() -> URL { URL(string: "noatinwork://sessions?filter=attention")! }
    static func status() -> URL { URL(string: "noatinwork://status")! }
    static func settings() -> URL { URL(string: "noatinwork://settings")! }
    static func changes() -> URL { URL(string: "noatinwork://changes")! }
    static func files() -> URL { URL(string: "noatinwork://view/files")! }
    static func instances() -> URL { URL(string: "noatinwork://view/instances")! }
    static func session(_ id: String) -> URL {
        let encoded = id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? id
        return URL(string: "noatinwork://session/\(encoded)") ?? newSession()
    }
}
...
// MARK: - Logo (full NoatinWork mark drawn from the SVG)

/// The NoatinWork logo, drawn to match packages/web/public/logo-dark-512x512.svg: an

...
const CHAT_FORCE_SCROLL_BOTTOM_EVENT = 'noatinwork:chat-force-scroll-bottom';
...
export const ChatContainer: React.FC<ChatContainerProps> = ({ active = true, autoOpenDraft = true, readOnly = false }) => {
    ...
    React.useEffect(() => {
        ...
        const scopedWindow = window as typeof window & {
            __noatinworkApplyChatSettingsSync?: (payload: { allowPromptingSubagentSessions: boolean }) => void;
        };
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if (data?.type !== 'noatinwork:chat-settings-sync'
        ...
        scopedWindow.__noatinworkApplyChatSettingsSync = applySync;
        window.addEventListener('message', handleMessage);
        parentWindow.postMessage({ type: 'noatinwork:chat-settings-request' }, window.location.origin);
        return () => {
            window.removeEventListener('message', handleMessage);
            if (scopedWindow.__noatinworkApplyChatSettingsSync === applySync) {
                delete scopedWindow.__noatinworkApplyChatSettingsSync;
            }
        };
    }, []);

...
const MODELS_DEV_PROXY_URL = "/api/noatinwork/models-metadata";
...
const fetchNoatinWorkDefaults = async (): Promise<NoatinWorkDefaults> => {
    markStartupTrace('config.defaults:start');
    const started = typeof performance !== 'undefined' ? performance.now() : Date.now();
    const finish = (source: string, result: NoatinWorkDefaults) => {
...
interface ConfigStore {
    ...
    // NoatinWork settings-based defaults (take precedence over agent preferences)
        ...
        persist(
            (set, get) => ({
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and NoatinWork settings in parallel. OpenCode config
                            ...
                            const [agents, noatinWorkDefaults] = await Promise.all([
                                measureStartupTrace(
                                    'loadAgents:api',
                                    () => opencodeClient.listAgents(configDirectoryPath),
                                    { directoryKey, source, requestedDirectory, effectiveDirectory, attempt: attempt + 1 },
                                ),
                                fetchNoatinWorkDefaults(),
                            ]);
                            ...
                            const defaultZenModel = normalizeOptionalString(noatinWorkDefaults.zenModel);
                            ...
                            set((state) => {
                                ...
                                const nextState: Partial<ConfigStore> = {
                                    settingsDefaultModel: noatinWorkDefaults.defaultModel,
                                    settingsDefaultVariant: noatinWorkDefaults.defaultVariant,
                                    settingsDefaultAgent: noatinWorkDefaults.defaultAgent,
                                    settingsAutoCreateWorktree: noatinWorkDefaults.autoCreateWorktree ?? false,
                                    settingsGitmojiEnabled: noatinWorkDefaults.gitmojiEnabled ?? false,
                                    settingsDefaultFileViewerPreview: noatinWorkDefaults.defaultFileViewerPreview ?? false,
                                    settingsZenModel: resolvedZenModel,
                                    settingsMessageStreamTransport: noatinWorkDefaults.messageStreamTransport ?? state.settingsMessageStreamTransport ?? 'auto',
                                    sttProvider: noatinWorkDefaults.sttProvider ?? state.sttProvider,
                                    sttServerUrl: noatinWorkDefaults.sttServerUrl ?? state.sttServerUrl,
                                    sttModel: noatinWorkDefaults.sttModel ?? state.sttModel,
                                    sttLocalModel: noatinWorkDefaults.sttLocalModel ?? state.sttLocalModel,
                                    sttLanguage: noatinWorkDefaults.sttLanguage ?? state.sttLanguage,
                                    directoryScoped: {
                                        ...state.directoryScoped,
                                        [directoryKey]: nextSnapshot,
                                    },
                            ...
                            // Detect invalid NoatinWork settings so we can clear them from storage.
                            ...
                            if (noatinWorkDefaults.defaultAgent && !safeAgents.some((agent) => agent.name === noatinWorkDefaults.defaultAgent)) {
                                invalidSettings.defaultAgent = '';
                            }
                            if (noatinWorkDefaults.defaultModel) {
                                const parsed = parseModelString(noatinWorkDefaults.defaultModel);
                                if (!parsed || !validateModel(parsed.providerId, parsed.modelId)) {
                                    invalidSettings.defaultModel = '';
                                } else if (noatinWorkDefaults.defaultVariant) {
                                    const provider = providers.find((p) => p.id === parsed.providerId);
                                    const model = provider?.models.find((m) => m.id === parsed.modelId) as { variants?: Record<string, unknown> } | undefined;
                                    const variants = model?.variants;
                                    if (!(variants && Object.prototype.hasOwnProperty.call(variants, noatinWorkDefaults.defaultVariant))) {
                ...
                setAgent: (agentName: string | undefined) => {
                    ...
                    if (agentName) {
                        ...
                        if (currentSessionId && useSessionUIStore.getState().isNoatinWorkCreatedSession(currentSessionId)) {
                            const existingAgentModel = selState.getAgentModelForSession(currentSessionId, agentName);
                            if (!existingAgentModel) {
                                useSessionUIStore.getState().initializeNewNoatinWorkSession(currentSessionId, agents);
                            }
                        }
                    }

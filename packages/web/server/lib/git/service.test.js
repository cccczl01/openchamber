...
const createTempDir = () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'noatinwork-git-service-'));
  tempDirs.push(dir);
  return dir;
};
  ...
  it('runs the post-checkout hook after populating a created worktree', async () => {
    ...
    try {
      ...
      const created = await createWorktree(repo, {
        ...
        branchName: 'noatinwork/hook-test',
        returnAfterDirectoryCreated: true,
      });
  ...
  it('skips a non-executable post-checkout hook', async () => {
    ...
    try {
      ...
      const created = await createWorktree(repo, {
        ...
        branchName: 'noatinwork/hook-skip-test',
        returnAfterDirectoryCreated: true,
      });
  ...
  it('does not fail worktree bootstrap when the post-checkout hook fails', async () => {
    ...
    try {
      ...
      const created = await createWorktree(repo, {
        ...
        branchName: 'noatinwork/hook-fail-test',
        returnAfterDirectoryCreated: true,
      });

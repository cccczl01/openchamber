...
function App({ apis }: AppProps) {
  React.useEffect(() => {
    ...
    if (startupTraceEnabled()) {
      console.info('[startup-trace] enabled. Run console.table(window.__NOATINWORK_STARTUP_TRACE__) after startup.');
    }
  }, []);
  ...
  React.useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'noatinwork:embedded-visibility') {
        return;
      }
    ...
    const scopedWindow = window as unknown as {
      __noatinworkSetEmbeddedVisibility?: (payload?: EmbeddedVisibilityPayload) => void;
    };

    scopedWindow.__noatinworkSetEmbeddedVisibility = applyVisibility;
    window.addEventListener('message', handleMessage);

    return () => {
      window.removeEventListener('message', handleMessage);
      if (scopedWindow.__noatinworkSetEmbeddedVisibility === applyVisibility) {
        delete scopedWindow.__noatinworkSetEmbeddedVisibility;
      }
    };
  }, [embeddedSessionChat]);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('noatinwork:open-session', handler as EventListener);
    return () => window.removeEventListener('noatinwork:open-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('noatinwork:open-mini-chat', onOpenMiniChat);
    return () => window.removeEventListener('noatinwork:open-mini-chat', onOpenMiniChat);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('noatinwork:open-draft-session', handler as EventListener);
    return () => window.removeEventListener('noatinwork:open-draft-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    (window as unknown as { __noatinworkAppReady?: boolean }).__noatinworkAppReady = true;
    window.dispatchEvent(new Event('noatinwork:app-ready'));
  ...
  // The Rust backend sets window.__NOATINWORK_DESKTOP_BOOT_OUTCOME__ once the

...
Rendering and hosting still happen in `noatinwork-website` (`apps/docs`).
...
- creates `noatinwork-docs-source-<sha>.tar.gz`
...
to `noatinwork-website` so it re-syncs and redeploys the docs. This fires on
...
- `NOATINWORK_WEBSITE_REPO_TOKEN` — a token with `contents: write` (classic
  `repo` scope, or fine-grained with Contents: read & write) on
  `cccczl01/noatinwork-website`. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from `noatinwork` (`main` on a
  push, the tag on a release)
- `archive_name`

`noatinwork-website`'s `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
`noatinwork` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
to Cloudflare Pages. That repo needs its own secrets: `NOATINWORK_REPO_TOKEN`

...
import { mintOutsideFileGrant } from '@noatinwork-app/web/server/lib/fs/routes.js';

const execFileAsync = promisify(execFile);

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const isDev = process.env.NOATINWORK_ELECTRON_DEV === '1' || !app.isPackaged;
const electronStartupStartedAt = performance.now();

const DEEP_LINK_PROTOCOL = 'noatinwork';
const UI_PROTOCOL = 'noatinwork-ui';
const PACKAGED_APP_USER_MODEL_ID = 'dev.noatinwork.desktop';
const DEV_APP_USER_MODEL_ID = 'dev.noatinwork.desktop.dev';
...
// ~/Library/Logs/NoatinWork/ (not ~/Library/Logs/@noatinwork-app/electron/).
app.setName('NoatinWork');
if (process.platform === 'linux') {
  app.setDesktopName('noatinwork.desktop');
}
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'NoatinWork Dev'));
}
...
if (shouldIgnoreLoopbackConnectionLimit({
  development: isDev,
  packagedUi: process.env.NOATINWORK_ELECTRON_USE_BUNDLED_UI === '1',
})) {
...
// that output never lands in ~/Library/Logs/NoatinWork/main.log and we
...
const recordElectronStartupPerformance = (phase, details = {}) => {
  const enabled = STARTUP_PERF_ENABLED_VALUES.has(String(process.env.NOATINWORK_STARTUP_PERF ?? '').toLowerCase());
...
const readAppMetadata = () => {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (parsed?.name === '@noatinwork-app/electron' && typeof parsed.version === 'string') {
        return { name: parsed.name, version: parsed.version };
      }
    } catch {
    }
  }
  return { name: '@noatinwork-app/electron', version: app.getVersion() };
};
...
const CHANGELOG_URL = 'https://raw.githubusercontent.com/ccccczl01/noatinwork/main/CHANGELOG.md';
const GITHUB_BUG_REPORT_URL = 'https://github.com/ccccczl01/noatinwork/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/ccccczl01/noatinwork/issues/new?template=feature_request.yml';
...
const quitConfirmationMessage = () => {
  ...
  return `NoatinWork detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
const requestQuitWithConfirmation = async () => {
  ...
  try {
    const result = await dialog.showMessageBox({
      type: 'warning',
      title: 'Quit NoatinWork?',
      message: 'Quit NoatinWork?',
...
const refreshQuitRiskFlags = async () => {
  ...
  const scheduledUrl = `${base}/api/noatinwork/scheduled-tasks/status`;
  const tunnelUrl = `${base}/api/noatinwork/tunnel/status`;
...
const settingsFilePath = () => {
  if (typeof process.env.NOATINWORK_DATA_DIR === 'string' && process.env.NOATINWORK_DATA_DIR.trim()) {
    return path.join(process.env.NOATINWORK_DATA_DIR.trim(), 'settings.json');
  }
  return path.join(os.homedir(), '.config', 'noatinwork', 'settings.json');
};
...
// session expires) reuses the same "NoatinWork Desktop" record instead of
...
const shouldUsePackagedUi = () => {
  if (process.env.NOATINWORK_ELECTRON_LOAD_SERVER_UI === '1') return false;
  if (process.env.NOATINWORK_ELECTRON_USE_BUNDLED_UI === '1') return true;
...
const injectRuntimeConfigIntoHtml = (html) => {
  ...
  const initScript = `<script>if(window.__NOATINWORK_LOCAL_ORIGIN__===undefined){window.__NOATINWORK_LOCAL_ORIGIN__=${JSON.stringify(localOrigin)};}if(window.__NOATINWORK_API_BASE_URL__===undefined){window.__NOATINWORK_API_BASE_URL__=${JSON.stringify(apiBaseUrl)};}if(window.__NOATINWORK_CLIENT_TOKEN__===undefined&&${JSON.stringify(state.clientToken || '')}){window.__NOATINWORK_CLIENT_TOKEN__=${JSON.stringify(state.clientToken || '')};}</script>`;
...
const maybeShowNativeNotification = (rawInput) => {
  ...
  notification.on('click', () => {
    ...
    if (sessionId) {
      emitToAllWindows('noatinwork:open-session', { sessionId, directory });
    }
    release();
  });
...
import { pathLooksUserConfigured, mergePathValues } from '@noatinwork-app/web/server/lib/opencode/path-utils.js';
import { clearAppImageArgv0FromProcessEnv } from '@noatinwork-app/web/server/lib/inherited-env.js';
...
const shouldSkipLocalServer = () => {
  inheritUserShellEnv();
  return process.env.NOATINWORK_SKIP_LOCAL_SERVER === '1';
};
...
const spawnLocalServer = async () => {
  ...
  // The server module reads ENV_DESKTOP_NOTIFY / NOATINWORK_DIST_DIR /
  // NOATINWORK_RUNTIME at import time (top-level const), so these must be
  ...
  process.env.NOATINWORK_HOST = bindHost;
  process.env.NOATINWORK_DESKTOP_LAN_ACCESS_ACTIVE = effectiveLanAccessEnabled ? 'true' : 'false';
  if (lanAccessBlockedByMissingPassword) {
    process.env.NOATINWORK_DESKTOP_LAN_ACCESS_BLOCKED_REASON = 'missing-password';
  } else {
    delete process.env.NOATINWORK_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
  }
  process.env.NOATINWORK_DIST_DIR = resolveWebDistDir();
  process.env.NOATINWORK_RUNTIME = 'desktop';
  ...
  process.env.NOATINWORK_OPENCODE_CWD = resolveManagedOpenCodeCwd({
    env: process.env,
    homedir: () => os.homedir(),
  });
  process.env.NOATINWORK_DESKTOP_NOTIFY = 'true';
  if (desktopUiPassword) {
    process.env.NOATINWORK_UI_PASSWORD = desktopUiPassword;
  } else {
    delete process.env.NOATINWORK_UI_PASSWORD;
  }
  process.env.NOATINWORK_SKIP_API_COMPRESSION = process.env.NOATINWORK_SKIP_API_COMPRESSION || 'true';
  process.env.NO_PROXY = process.env.NO_PROXY || 'localhost,127.0.0.1';
  process.env.no_proxy = process.env.no_proxy || 'localhost,127.0.0.1';

  const { startWebUiServer } = await import('@noatinwork-app/web/server/index.js');
...
`;
  ...
  const child = spawn('/bin/sh', ['-c', script, 'noatinwork-opencode-killer', normalizedPid, normalizedPort, String(OPENCODE_SHUTDOWN_GRACE_MS / 1000)], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
  });
...
const buildInitScript = (localOrigin, bootOutcome, apiBaseUrl = '', clientToken = '', requestHeaders = {}) => {
  ...
  return [
    '(function(){',
    `try{var __oc_local=${local};var __oc_api=${apiBase};var __oc_headers=${headers};var __oc_packaged=${packagedOrigin};var __oc_origin=window.location&&window.location.origin||'';var __oc_is_packaged=__oc_origin===__oc_packaged;var __oc_is_local=__oc_local&&__oc_origin===new URL(__oc_local).origin;window.__NOATINWORK_MACOS_MAJOR__=${macVersion};window.__NOATINWORK_LOCAL_ORIGIN__=__oc_local;window.__NOATINWORK_API_BASE_URL__=__oc_api;if(__oc_is_local||__oc_is_packaged){window.__NOATINWORK_HOME__=${home};window.__NOATINWORK_RUNTIME_HEADERS__=__oc_headers;}if((__oc_is_local||__oc_is_packaged)&&${token}){window.__NOATINWORK_CLIENT_TOKEN__=${token};}var __oc_bo=${outcome};if(__oc_bo){window.__NOATINWORK_DESKTOP_BOOT_OUTCOME__=__oc_bo;}}catch(_e){}`,
    '}())',
  ].join('');
};
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="NoatinWork loading icon">
...
const loginRemoteAndIssueClientToken = async ({ url, password, trustDevice, requestHeaders }) => {
  ...
  const loginResponse = await fetch(new URL('/auth/session', `${baseUrl}/`).toString(), {
    ...
    body: JSON.stringify({
      ...
      clientLabel: 'NoatinWork Desktop',
      ...clientIdentity,
    }),
  });
  if (!loginResponse.ok) {
...
const emitToWindow = (browserWindow, event, detail) => {
  if (!browserWindow || browserWindow.isDestroyed()) return;
  browserWindow.webContents.send('noatinwork:emit', { event, detail });
};
...
const parseConnectPairingDeepLinkPayload = (raw) => {
  ...
  try {
    ...
    return {
      pairingId,
      secret,
      label: typeof payload.label === 'string' && payload.label.trim() ? payload.label.trim() : 'NoatinWork',
...
const confirmConnectDeepLink = async (payload) => {
  ...
  const options = {
    type: 'warning',
    title: 'Connect to NoatinWork server?',
...
const dispatchDeepLink = (link) => {
  ...
  if (link.type === 'focus') {
    ...
    emitToAllWindows('noatinwork:deep-link-focus', { reason: link.value || null });
    return;
  }

  if (link.type === 'session' && link.value) {
    emitToAllWindows('noatinwork:open-session', { sessionId: link.value });
    return;
  }
...
const dispatchMenuAction = (action) => {
  ...
  emitToWindow(target, 'noatinwork:menu-action', action);
  dispatchDomEventToWindow(target, 'noatinwork:menu-action', action);
};
...
const dispatchAddSelectionToChat = () => {
  const target = getMenuTargetWindow();
  if (target) emitToWindow(target, 'noatinwork:menu-action', 'add-selection-to-chat');
};
...
const dispatchOpenMiniChat = (browserWindow) => {
  ...
  if (target) emitToWindow(target, 'noatinwork:open-mini-chat');
};

const dispatchCheckForUpdates = () => {
  emitToAllWindows('noatinwork:check-for-updates');
  for (const browserWindow of BrowserWindow.getAllWindows()) {
    dispatchDomEventToWindow(browserWindow, 'noatinwork:check-for-updates');
  }
};
...
const createBrowserWindow = ({ label, restoreGeometry, url, runtimeConfig = {} }) => {
  ...
  const options = {
    ...
    webPreferences: {
      additionalArguments: [
        `--noatinwork-local-origin=${desktopLocalOrigin}`,
        `--noatinwork-api-base-url=${desktopApiBaseUrl}`,
        `--noatinwork-client-token=${desktopClientToken}`,
        `--noatinwork-runtime-headers=${JSON.stringify(desktopRequestHeaders)}`,
        `--noatinwork-home=${desktopHome}`,
        `--noatinwork-macos-major=${desktopMacosMajor}`,
        `--noatinwork-tray-enabled=${trayEnabled ? '1' : '0'}`,
        `--noatinwork-boot-outcome=${JSON.stringify(state.bootOutcome || null)}`,
        `--noatinwork-relay-host-id=${rendererRuntimeConfig.relayHostId || ''}`,
      ],
  ...
  browserWindow.on('resize', () => {
    if (process.platform === 'darwin') {
      emitToWindow(browserWindow, 'noatinwork:window-resized');
    }
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('maximize', () => {
    emitToWindow(browserWindow, 'noatinwork:window-maximized-changed', { maximized: true });
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('unmaximize', () => {
    emitToWindow(browserWindow, 'noatinwork:window-maximized-changed', { maximized: false });
    debounceWindowStatePersist(browserWindow, false);
  });
...
const resolveInitialUrl = async () => {
  const hmrApiPort = process.env.NOATINWORK_HMR_API_PORT || '3901';
  const hmrUiPort = process.env.NOATINWORK_HMR_UI_PORT || '5173';
  ...
  const envTarget = normalizeHostUrl(process.env.NOATINWORK_SERVER_URL || '');
  const config = readDesktopHostsConfig();
  if (envTarget) {
  ...
  if (!initialUrl) {
    throw new Error(
      'NOATINWORK_SKIP_LOCAL_SERVER=1 requires bundled UI, a running desktop HMR UI, or a reachable remote instance.',
    );
  }
...
const setupAutoUpdater = () => {
  ...
  const testBuild = typeof __NOATINWORK_UPDATER_E2E_BUILD__ !== 'undefined'
    && __NOATINWORK_UPDATER_E2E_BUILD__ === true;
  ...
  autoUpdater.on('download-progress', (progress) => {
    ...
    emitToAllWindows('noatinwork:update-progress', mapUpdaterProgressEvent({
      event: 'Progress',
      data: {
...
const iconToDataUrl = async (iconPath, appName) => {
  ...
  const tempPath = path.join(os.tmpdir(), `noatinwork-icon-${safeName}-${Date.now()}.png`);
  try {
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    case 'desktop_read_file': {
      ...
      const DENIED_SEGMENTS = ['.ssh', '.aws', '.gnupg', '.gpg', '.config/gh', '.config/noatinwork/credentials'];
    ...
    case 'desktop_get_installed_apps': {
      ...
      const refresh = async () => {
        ...
        emitToAllWindows('noatinwork:installed-apps-updated', apps);
      };
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires NoatinWork.app to be installed in /Applications');
          }
        } catch (error) {
    ...
    case 'desktop_focus_main_window': {
      ...
      if (sessionId) {
        emitToWindow(state.mainWindow, 'noatinwork:open-session', { sessionId, directory });
      } else if (mode === 'draft') {
        emitToWindow(state.mainWindow, 'noatinwork:open-draft-session', { directory, projectId });
      }
      return { focused: true };
    }
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About NoatinWork', click: () => dispatchAction('about') },
        {
...
const isLocalSender = (webContents) => {
  try {
    ...
    if (isDev && url.origin === `http://127.0.0.1:${process.env.NOATINWORK_HMR_UI_PORT || '5173'}`) return true;
    if (url.protocol !== 'http:' && url.protocol !== 'https:') return false;
    if (state.localOrigin) {
      try {
...
ipcMain.handle('noatinwork:invoke', async (event, command, args) => {
...
ipcMain.handle('noatinwork:dialog:open', async (event, options) => {
...
ipcMain.handle('noatinwork:file:grant-existing', async (event, filePath) => {
  if (!isLocalSender(event.sender)) {
...
const focusMainWindowWithSession = async (sessionId, directory) => {
  if (state.mainWindow && !state.mainWindow.isDestroyed()) {
    ...
    state.mainWindow.focus();
    if (sessionId) {
      emitToWindow(state.mainWindow, 'noatinwork:open-session', { sessionId, directory: directory || '' });
    }
    return;
  }
  if (sessionId) pendingDeepLinks.push({ type: 'session', value: sessionId });
...
const dispatchTrayAction = async (action) => {
  ...
  if (action.type === 'respond-permission') {
    const target = (state.mainWindow && !state.mainWindow.isDestroyed())
      ? state.mainWindow
      : await revealMainWindow();
    emitToWindow(target, 'noatinwork:tray-action', action);
    return;
  }
  ...
  if (action.type === 'focus-session') {
    ...
    if (surface && surface.__ocMiniChat === true && action.sessionId) {
      ...
      surface.show();
      surface.focus();
      emitToWindow(surface, 'noatinwork:open-session', {
  ...
  if (action.type === 'new-session') {
    emitToWindow(target, 'noatinwork:open-draft-session', { directory: '', projectId: '' });
  }
...
app.whenReady().then(async () => {
  ...
  powerMonitor.on('resume', () => {
    emitToAllWindows('noatinwork:system-resume', { timestamp: Date.now() });
  });
}).catch((error) => {

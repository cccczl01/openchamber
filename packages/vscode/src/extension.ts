...
const SETTINGS_KEY = 'noatinwork.settings';
...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('NoatinWork');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      await vscode.commands.executeCommand('noatinwork.chatView.focus');
      await vscode.commands.executeCommand(moveCommandId);
      return 'moved';
    } catch (error) {
      outputChannel?.appendLine(
        `[NoatinWork] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  const maybeMoveChatToRightSidebarOnStartup = async () => {
    ...
    const attempted = context.globalState.get<boolean>('noatinwork.sidebarAutoMoveAttempted') || false;
    if (attempted) return;
    await context.globalState.update('noatinwork.sidebarAutoMoveAttempted', true);
  ...
  const config = vscode.workspace.getConfiguration('noatinwork');
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.openSidebar', async () => {
      // Best-effort: open the container (if available), then focus the chat view.
      try {
        await vscode.commands.executeCommand('workbench.view.extension.noatinwork');
      } catch (e) {
        outputChannel?.appendLine(`[NoatinWork] workbench.view.extension.noatinwork failed: ${e}`);
      }

      try {
        await vscode.commands.executeCommand('noatinwork.chatView.focus');
      } catch (e) {
        outputChannel?.appendLine(`[NoatinWork] noatinwork.chatView.focus failed: ${e}`);
        vscode.window.showErrorMessage(t('NoatinWork: Failed to open sidebar - {0}', String(e)));
        return false;
      }

      if (!chatViewProvider?.hasResolvedView()) {
        outputChannel?.appendLine('[NoatinWork] Chat sidebar focus completed before the webview was resolved');
        vscode.window.showWarningMessage(t('NoatinWork: Chat sidebar is not ready'));
        return false;
      }
  ...
  const revealChatViewForPayload = async () => {
    const opened = await vscode.commands.executeCommand<boolean>('noatinwork.openSidebar');
    if (!opened) {
      return false;
    }
    ...
    if (!chatViewProvider?.hasResolvedView()) {
      outputChannel?.appendLine('[NoatinWork] Chat sidebar webview was disposed before payload delivery');
      vscode.window.showWarningMessage(t('NoatinWork: Chat sidebar is not ready'));
      return false;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.focusChat', async () => {
      await vscode.commands.executeCommand('noatinwork.chatView.focus');
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.internal.settingsSynced', (settings: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.internal.permissionAutoAcceptSynced', (snapshot: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.openAgentManager', () => {
      agentManagerProvider?.createOrShow();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.setActiveSession', (sessionId: unknown, title?: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage(t('NoatinWork: No active session'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.openSessionInEditor', (sessionId: string, title?: string) => {
      if (typeof sessionId !== 'string' || sessionId.trim().length === 0) {
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.openNewSessionInEditor', () => {
      sessionEditorProvider?.createOrShowNewSession();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.openCurrentOrNewSessionInEditor', () => {
      if (activeSessionId) {
        sessionEditorProvider?.createOrShow(activeSessionId, activeSessionTitle ?? undefined);
      } else {
        sessionEditorProvider?.createOrShowNewSession();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.restartApi', async () => {
      try {
        ...
        vscode.window.showInformationMessage(t('NoatinWork: API connection restarted'));
      } catch (e) {
        vscode.window.showErrorMessage(t('NoatinWork: Failed to restart API - {0}', String(e)));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.addToContext', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('NoatinWork [Add to Context]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('NoatinWork [Add to Context]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (attachedFiles.length === 0) {
        vscode.window.showWarningMessage(t('NoatinWork: No file selected to mention'));
        return;
      }
      ...
      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage(t('NoatinWork: Some selected entries were skipped (folders or unsupported resources)'));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('NoatinWork [Explain]: No active editor'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('NoatinWork [Improve Code]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('NoatinWork [Improve Code]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.newSession', async (directory?: unknown) => {
      ...
      if (!folderPath && candidates.length === 0) {
        vscode.window.showInformationMessage('NoatinWork: No folder is open. Open a folder to start a new session.');
        return;
      }
      ...
      if (openCodeManager) {
        ...
        if (!result.success) {
          vscode.window.showErrorMessage(`NoatinWork: ${result.error}`);
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.showSettings', () => {
      chatViewProvider?.showSettings();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('noatinwork.showOpenCodeStatus', async () => {
      const config = vscode.workspace.getConfiguration('noatinwork');
      ...
      const lines = [
        ...
        `NoatinWork version: ${extensionVersion || '(unknown)'}`,
        ...
        `OpenCode binary (configured): ${(vscode.workspace.getConfiguration('noatinwork').get<string>('opencodeBinary') || '').trim() || '(none)'}`,

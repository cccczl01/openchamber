 ...
 *   NOATINWORK_ELECTRON_PKG_DIR       - resolve the electron package here.
 *   NOATINWORK_ELECTRON_INSTALL_COMMANDS - JSON array of [bin, args] repair
 *                                        commands, e.g.
...
function resolveInstallCommands(env) {
  if (env.NOATINWORK_ELECTRON_INSTALL_COMMANDS) {
    try {
      const parsed = JSON.parse(env.NOATINWORK_ELECTRON_INSTALL_COMMANDS);
      if (
...
export async function main(argv = process.argv.slice(2), env = process.env) {
  ...
  const overrideDir = env.NOATINWORK_ELECTRON_PKG_DIR;

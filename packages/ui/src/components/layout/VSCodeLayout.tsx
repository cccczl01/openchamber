...
export const VSCodeLayout: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () => (typeof window !== 'undefined'
      ? (window as { __NOATINWORK_CONNECTION__?: { status?: string } }).__NOATINWORK_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('noatinwork.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('noatinwork.updateSessionEditorTitle', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as { __NOATINWORK_CONNECTION__?: { status?: string } }).__NOATINWORK_CONNECTION__?.status
    ...
    window.addEventListener('noatinwork:connection-status', handler as EventListener);
    return () => window.removeEventListener('noatinwork:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('noatinwork:navigate', handler as EventListener);
    return () => window.removeEventListener('noatinwork:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('noatinwork_stream_debug') === '1';
          } catch {
            return false;
          }
        })();

        if (debugEnabled) console.log('[NoatinWork][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[NoatinWork][VSCode][bootstrap] post-load', {

...
files:
  - url: NoatinWork-1.2.3-win-${architecture}.exe
...
const createFixture = ({ includeArm64 = true } = {}) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'noatinwork-latest-yml-'));
...
const environment = ({ artifacts, output }) => ({
  ...
  RUNNER_TEMP: output,
  GH_REPO: 'ccccczl01/noatinwork',
  NOATINWORK_VERSION: '1.2.3',
});

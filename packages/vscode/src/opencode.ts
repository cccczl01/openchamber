...
function readNoatinWorkSettings(): Record<string, unknown> {
  const settingsPath = path.join(os.homedir(), '.config', 'noatinwork', 'settings.json');
  try {
...
function createConfiguredOpencodeBinaryError(raw: string, normalized: string): Error {
  const messageSuffix = 'NoatinWork needs the standalone opencode CLI. Install it and set noatinwork.opencodeBinary to the CLI path, for example ~/.opencode/bin/opencode, or leave the setting empty to use PATH lookup.';
...
function validateConfiguredOpencodeBinaryForManagedStart(): string | null {
  ...
  try {
    const settings = readNoatinWorkSettings();
...
function resolveOpencodeCliPath(): string | null {
  ...
  if (sharedFromNoatinWork && isExecutable(sharedFromNoatinWork) && !isKnownOpenCodeDesktopAppPath(sharedFromNoatinWork)) {
    return sharedFromNoatinWork;
  }
...
async function waitForReady(
  ...
  const outputChannel = vscode.window.createOutputChannel('NoatinWorkManager');
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    const onExit = (code: number | null) => {
      ...
      const appBundleHint = isMacOpenCodeAppBundlePath(binary)
        ? ' The configured binary appears to point at the macOS desktop app bundle; NoatinWork needs the standalone opencode CLI.'
        : '';

...
// client, and the /api/noatinwork/relay/* management routes.
//
...
// Routes are registered with the other NoatinWork feature routes, before the
...
export const DEFAULT_RELAY_URL = 'wss://relay.noatinwork.dev/ws';
...
const envRelayUrlOverride = () => {
  const raw = process.env.NOATINWORK_RELAY_URL;
...
export const createRelayService = ({
  ...
  const readConfig = async () => {
    ...
    return {
      ...
      // True when the endpoint is pinned by NOATINWORK_RELAY_URL (a self-hosted
  ...
  const standbyStatus = (holderPid) => ({
    state: 'standby',
    lastError: `relay host is owned by another local NoatinWork process (pid ${holderPid})`,
    connectedClients: 0,
  });
  ...
  const registerRoutes = (app) => {
    app.get('/api/noatinwork/relay/status', async (_req, res) => {
      try {
    ...
    app.post('/api/noatinwork/relay/enable', express.json({ limit: '16kb' }), async (req, res) => {
      try {
    ...
    app.post('/api/noatinwork/relay/disable', async (_req, res) => {
      try {
        const current = await readConfig();

...
vi.mock('@noatinwork-app/ui/lib/runtime-auth', () => ({
...
vi.mock('@noatinwork-app/ui/lib/runtime-fetch', () => ({ installRuntimeFetchBridge: vi.fn() }));
vi.mock('@noatinwork-app/ui/lib/runtime-switch', () => ({
  getRuntimeApiBaseUrl: vi.fn(() => ''),
  getRuntimeKey: vi.fn(() => 'local'),
  initializeRuntimeEndpoint: vi.fn(),
  switchRuntimeEndpoint: vi.fn(),
}));
vi.mock('@noatinwork-app/ui/lib/desktopRelayRestore', () => ({ restoreDesktopRelayRuntime: vi.fn(() => Promise.resolve()) }));
vi.mock('@noatinwork-app/ui/lib/runtime-url', () => ({ configureRuntimeUrlResolver: vi.fn(() => ({})) }));
vi.mock('@noatinwork-app/ui/lib/opencode/client', () => ({ opencodeClient: { reconnectToRuntimeBaseUrl: vi.fn() } }));
vi.mock('./api', () => ({ createWebAPIs: vi.fn() }));

import { setRuntimeBearerToken, setRuntimeExtraHeaders } from '@noatinwork-app/ui/lib/runtime-auth';
import { initializeRuntimeEndpoint, switchRuntimeEndpoint } from '@noatinwork-app/ui/lib/runtime-switch';
import { restoreDesktopRelayRuntime } from '@noatinwork-app/ui/lib/desktopRelayRestore';
...
const makeWindow = (search = ''): Record<string, unknown> => {
  const value: Record<string, unknown> = {
    location: { origin: 'noatinwork-ui://app', search },
    setTimeout: vi.fn(() => 1),
  };
  value.parent = value;
  return value;
...
describe('readRuntimeBootstrapConfig', () => {
  test('reads the runtime injected into the current window', () => {
    ...
    current.__NOATINWORK_API_BASE_URL__ = ' https://remote.example.com ';
    current.__NOATINWORK_CLIENT_TOKEN__ = ' remote-token ';
    current.__NOATINWORK_LOCAL_ORIGIN__ = ' http://127.0.0.1:3000 ';
    current.__NOATINWORK_RUNTIME_HEADERS__ = { 'x-noatinwork-relay': 'relay-value' };
    current.__NOATINWORK_RELAY_HOST_ID__ = ' remote-host ';
    ...
    expect(readRuntimeBootstrapConfig()).toEqual({
      ...
      runtimeHeaders: { 'x-noatinwork-relay': 'relay-value' },
  ...
  test('does not read runtime credentials directly from a parent window', () => {
    ...
    parent.__NOATINWORK_API_BASE_URL__ = 'https://remote.example.com';
    parent.__NOATINWORK_CLIENT_TOKEN__ = 'remote-token';

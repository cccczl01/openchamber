...
import { resolveHostedSurface, type HostedSurface } from '@noatinwork-app/ui/lib/runtimeSurface';
import {
  isEmbeddedSessionChat,
  requestEmbeddedSessionRuntimeBootstrap,
} from '@noatinwork-app/ui/components/layout/contextPanelEmbeddedChat';
...
declare global {
  interface Window {
    ...
    __NOATINWORK_SURFACE__?: HostedSurface;
  }
}
...
const start = async (): Promise<void> => {
  const embeddedBootstrap = isEmbeddedSessionChat()
    ...
    : null;
  window.__NOATINWORK_RUNTIME_APIS__ = createConfiguredWebAPIs(embeddedBootstrap);

  if (hostedSurface === 'mobile') {
    const { renderMobileApp } = await import('@noatinwork-app/ui/apps/renderMobileApp');
    renderMobileApp(window.__NOATINWORK_RUNTIME_APIS__);
    return;
  }

  // Hold the render until a desktop relay-host restore has picked its transport.
  await getDesktopRelayRestoreReady();
  await import('@noatinwork-app/ui/main');
};

void start();

if (import.meta.hot) {
  import.meta.hot.on('noatinwork:theme-updated', (theme: unknown) => {
    window.dispatchEvent(new CustomEvent('noatinwork:theme-hmr', { detail: theme }));
  });
}

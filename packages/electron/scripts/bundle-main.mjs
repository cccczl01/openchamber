 ...
 * (@noatinwork-app/web) and native modules — stays external so it resolves
 ...
 * Leaving @noatinwork-app/web external means the conditional
...
const updaterE2eBuild = process.env.NOATINWORK_UPDATER_E2E_BUILD === '1';
...
const result = await Bun.build({
  ...
  naming: '[name].mjs',
  define: {
    __NOATINWORK_UPDATER_E2E_BUILD__: updaterE2eBuild ? 'true' : 'false',
  },
});

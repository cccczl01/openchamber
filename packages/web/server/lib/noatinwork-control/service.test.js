...
import { createNoatinWorkControlService } from './service.js';
...
const createService = (overrides = {}) => {
  ...
  const service = createNoatinWorkControlService({
...
describe('NoatinWork control service', () => {
  ...
  it('rejects actions outside the fixed contract', async () => {
    const { service } = createService();
    await expect(service.execute('session.delete')).rejects.toThrow('Unsupported NoatinWork action');
  });
});

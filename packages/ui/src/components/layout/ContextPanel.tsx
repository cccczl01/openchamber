...
const DESKTOP_BROWSER_INSPECT_SCRIPT = `new Promise((resolve) => {
  const existing = document.getElementById('__noatinwork_desktop_browser_overlay');
  if (existing) existing.remove();
  if (typeof window.__noatinworkDesktopBrowserCancelInspect === 'function') {
    try { window.__noatinworkDesktopBrowserCancelInspect(); } catch { /* webview not ready */ }
  }
  const overlay = document.createElement('div');
  overlay.id = '__noatinwork_desktop_browser_overlay';
  ...
  const cleanup = () => {
    ...
    if (window.__noatinworkDesktopBrowserCancelInspect === cancel) {
      delete window.__noatinworkDesktopBrowserCancelInspect;
    }
  };
  const cancel = () => {
    cleanup();
  ...
  window.__noatinworkDesktopBrowserCancelInspect = cancel;
...
const DESKTOP_BROWSER_CANCEL_INSPECT_SCRIPT = `(() => {
  if (typeof window.__noatinworkDesktopBrowserCancelInspect === 'function') {
    window.__noatinworkDesktopBrowserCancelInspect();
    return;
  }
  const overlay = document.getElementById('__noatinwork_desktop_browser_overlay');
  if (overlay) overlay.remove();
})()`;

const DESKTOP_BROWSER_SAME_WEBVIEW_NAVIGATION_SCRIPT = `(() => {
  if (window.__noatinworkSameWebviewNavigationInstalled) return;
  window.__noatinworkSameWebviewNavigationInstalled = true;
...
const PreviewPane: React.FC<PreviewPaneProps> = ({ rawUrl, onNavigate }) => {
  ...
  React.useEffect(() => {
    ...
    postPreviewBridgeMessage(frameWindow, proxySrc, {
      source: 'noatinwork-preview-parent',
      version: 1,
  ...
  React.useEffect(() => {
    ...
    const handler = (event: MessageEvent<PreviewBridgeMessage>) => {
      ...
      if (!data || data.source !== 'noatinwork-preview-bridge' || data.version !== 1) {
        return;
      }
...
const isElectronBrowserRuntime = (): boolean => {
  return typeof window !== 'undefined' && Boolean(window.__NOATINWORK_ELECTRON__);
};
...
const IframeBrowserPane: React.FC<DesktopBrowserPaneProps> = ({ initialUrl, directory, tabID }) => {
  ...
  React.useEffect(() => {
    const handler = (event: MessageEvent<PreviewBridgeMessage>) => {
      ...
      const data = event.data;
      if (!data || data.source !== 'noatinwork-preview-bridge' || data.version !== 1) return;
      ...
      <div className="relative min-h-0 flex-1 bg-background">
        {iframeSrc ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 bg-background p-6 text-center">
            <NoatinWorkLogo width={140} height={140} className="opacity-20" />
    ...
    <div className="absolute inset-0 flex flex-col bg-background">
      ...
      <div className="relative min-h-0 flex-1 bg-background">
        <webview
          ...
          partition="persist:noatinwork-browser"
          allowpopups
...
export const ContextPanel: React.FC = () => {
  ...
  const postChatSettingsSyncToEmbeddedChat = React.useCallback(() => {
    ...
    for (const frame of chatFrameRefs.current.values()) {
      ...
      frameWindow.postMessage({ type: 'noatinwork:chat-settings-sync', payload }, window.location.origin);
    }
  ...
  const postEmbeddedVisibilityToChats = React.useCallback(() => {
    ...
    for (const [tabID, frame] of chatFrameRefs.current.entries()) {
      ...
      frameWindow.postMessage(
        {
          type: 'noatinwork:embedded-visibility',
          payload,
        },
        window.location.origin,
      );
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type === EMBEDDED_RUNTIME_BOOTSTRAP_REQUEST) {
        ...
        const payload: EmbeddedSessionRuntimeBootstrap = {
          ...
          localOrigin: typeof window.__NOATINWORK_LOCAL_ORIGIN__ === 'string'
      ...
      if (data?.type === 'noatinwork:theme-sync-request') {
        postThemeSyncToEmbeddedChat();
        return;
      }
      if (data?.type === 'noatinwork:chat-settings-request') {
        postChatSettingsSyncToEmbeddedChat();
        return;
      }
      if (data?.type !== 'noatinwork:cycle-theme-request') {
        return;
      }

...
const sessionWith = (linked: unknown): Session =>
  ({ metadata: { noatinwork: { linked_issues: linked } } } as unknown as Session);
...
describe('withLinkedIssue', () => {
  test('adds a link and preserves unrelated metadata', () => {
    const next = withLinkedIssue(
      { noatinwork: { kind: 'review' }, other: 1 },
      issue(),
      true,
    );
    expect(next.other).toBe(1);
    expect((next.noatinwork as Record<string, unknown>).kind).toBe('review');
    expect((next.noatinwork as { linked_issues: LinkedIssue[] }).linked_issues).toEqual([issue()]);
  });
  ...
  test('re-linking replaces the entry rather than duplicating it', () => {
    ...
    const stored = (second.noatinwork as { linked_issues: LinkedIssue[] }).linked_issues;
  ...
  test('unlinking removes only the matching id', () => {
    ...
    const stored = (next.noatinwork as { linked_issues: LinkedIssue[] }).linked_issues;
    expect(stored).toEqual([other]);
  });

  test('unlinking something absent is a no-op, not an error', () => {
    const next = withLinkedIssue({}, issue(), false);
    expect((next.noatinwork as { linked_issues: LinkedIssue[] }).linked_issues).toEqual([]);
  });

  test('does not carry malformed stored entries forward', () => {
    const next = withLinkedIssue(
      { noatinwork: { linked_issues: [{ id: 'broken' }] } },
      issue(),
      true,
    );
    expect((next.noatinwork as { linked_issues: LinkedIssue[] }).linked_issues).toEqual([issue()]);

...
import { NoatinWorkControlError } from './error.js';
import { registerNoatinWorkControlRoutes } from './routes.js';

const createApp = (execute) => {
  const app = express();
  registerNoatinWorkControlRoutes(app, { controlService: { execute } });
  return app;
};

describe('NoatinWork control route', () => {
  ...
  it('preserves service status and partial-result details', async () => {
    const execute = vi.fn(async () => {
      throw new NoatinWorkControlError('dispatch failed', 500, {
        partial: true,

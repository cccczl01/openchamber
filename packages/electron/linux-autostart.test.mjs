...
test('prefers APPIMAGE path for Linux autostart Exec', () => {
  assert.equal(
    resolveLinuxLaunchExecutable({
      env: { APPIMAGE: '/home/user/NoatinWork.AppImage' },
      execPath: '/tmp/.mount_OpenChXXXX/noatinwork',
    }),
    '/home/user/NoatinWork.AppImage',
  );
});
...
test('writes and removes the XDG autostart file', async () => {
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'noatinwork-autostart-'));
  ...
  try {
    ...
    const enabled = await setLinuxAutostartEnabled({
      enabled: true,
      backgroundArg: '--background',
      env: { ...env, APPIMAGE: '/opt/NoatinWork.AppImage' },
      homeDir,
    });
    ...
    assert.match(contents, /Exec=\/opt\/NoatinWork\.AppImage --background/);

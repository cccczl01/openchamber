...
const PACKAGE_NAME = '@noatinwork-app/web';
const PACKAGE_PATH_SEGMENTS = PACKAGE_NAME.split('/');
const NPM_REGISTRY_URL = `https://registry.npmjs.org/${PACKAGE_NAME}`;
const CHANGELOG_URL = 'https://raw.githubusercontent.com/ccccczl01/noatinwork/main/CHANGELOG.md';
const GITHUB_RELEASES_URL = 'https://github.com/ccccczl01/noatinwork/releases';
const GITHUB_RELEASES_API_URL = 'https://api.github.com/repos/ccccczl01/noatinwork/releases';
let cachedDetectedPm = null;

function getSpawnSyncBaseOptions() {
  return process.platform === 'win32' ? { windowsHide: true } : {};
}
const UPDATE_CHECK_URL = process.env.NOATINWORK_UPDATE_API_URL || 'https://api.noatinwork.dev/v1/update/check';

function getNoatinWorkConfigDir() {
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA;
    if (appData) return path.join(appData, 'noatinwork');
  }

  return path.join(os.homedir(), '.config', 'noatinwork');
}
...
function getOrCreateInstallId(scope = 'web') {
  const configDir = getNoatinWorkConfigDir();
  ...
  try {
    const response = await fetch(`${GITHUB_RELEASES_API_URL}/tags/v${version}`, {
      headers: {
        ...
        'User-Agent': 'noatinwork-update-check',
      },
    ...
    const apkAssets = Array.isArray(release?.assets)
      ...
      : [];
    const canonicalAsset = apkAssets.find((asset) => /^NoatinWork-.+-android\.apk$/i.test(asset.name));
...
function getOwnedPackagePathsFromGlobalBins(pm) {
  ...
  for (const binDir of getGlobalBinDirs(pm)) {
    const binaryName = process.platform === 'win32' ? 'noatinwork.cmd' : 'noatinwork';
...
export function detectPackageManagerDetails() {
  ...
  const forcedPm = process.env.NOATINWORK_PACKAGE_MANAGER?.trim();
...
function isPackageInstalledWith(pm) {
  try {
    ...
    return result.stdout.includes(PACKAGE_NAME) || result.stdout.includes('noatinwork');
  } catch {
    return false;
  }
}
  ...
  if (currentVersion !== 'unknown') {
    ...
    if (remote) {
      ...
      return {
        ...remote,
        packageManager: pm,
        updateCommand: 'noatinwork update',
      };
    }
  }

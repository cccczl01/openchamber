...
export const PREVIEW_TARGET_ERROR_HEADER = 'x-noatinwork-preview-target-error';
...
const PREVIEW_BRIDGE_SCRIPT_ID = 'noatinwork-preview-bridge';
...
const PREVIEW_BRIDGE_SCRIPT = String.raw`(() => {
  if (window.__noatinworkPreviewBridgeInstalled) return;
  window.__noatinworkPreviewBridgeInstalled = true;

  const SOURCE = 'noatinwork-preview-bridge';
  const VERSION = 1;
  const MAX_TEXT = 500;
  const MAX_ARG = 1000;
  const TARGET_ORIGIN = typeof window.__noatinworkPreviewTargetOrigin === 'string' ? window.__noatinworkPreviewTargetOrigin : '';
  ...
  const installColorSchemeMatchMediaPatch = () => {
    if (window.__noatinworkPreviewColorSchemePatched || typeof window.matchMedia !== 'function') return;
    window.__noatinworkPreviewColorSchemePatched = true;
  ...
  const applyPreviewColorScheme = (scheme) => {
    ...
    try {
      ...
      root.dataset.noatinworkPreviewColorScheme = next;
  ...
  const installViteHmrProxyPatch = () => {
    if (window.__noatinworkViteHmrProxyPatched || typeof window.WebSocket !== 'function') return;
    window.__noatinworkViteHmrProxyPatched = true;
    ...
    function NoatinWorkPreviewWebSocket(url, protocols) {
      const protocolList = Array.isArray(protocols) ? protocols : [protocols];
    ...
    NoatinWorkPreviewWebSocket.prototype = NativeWebSocket.prototype;
    Object.setPrototypeOf(NoatinWorkPreviewWebSocket, NativeWebSocket);
    Object.defineProperty(NoatinWorkPreviewWebSocket, 'name', { value: 'WebSocket' });
    window.WebSocket = NoatinWorkPreviewWebSocket;
  };

  const installAppRequestProxyPatch = () => {
    if (window.__noatinworkAppRequestProxyPatched) return;
    window.__noatinworkAppRequestProxyPatched = true;
    const proxyMatch = window.location.pathname.match(/^(\/api\/preview\/proxy\/[a-f0-9]{16,64})(?:\/|$)/i);
    if (!proxyMatch) return;
    const proxyBase = proxyMatch[1];
    const currentSearchParams = new URL(window.location.href).searchParams;
    ...
    if (typeof window.EventSource === 'function') {
      ...
      function NoatinWorkPreviewEventSource(url, eventSourceInitDict) {
        return new NativeEventSource(proxiedUrl(String(url)), eventSourceInitDict);
      }
      NoatinWorkPreviewEventSource.prototype = NativeEventSource.prototype;
      Object.setPrototypeOf(NoatinWorkPreviewEventSource, NativeEventSource);
      Object.defineProperty(NoatinWorkPreviewEventSource, 'name', { value: 'EventSource' });
      window.EventSource = NoatinWorkPreviewEventSource;
    }

    if (typeof window.WebSocket === 'function') {
      const NativeWebSocket = window.WebSocket;
      function NoatinWorkPreviewAppWebSocket(url, protocols) {
        const nextUrl = proxiedWebSocketUrl(String(url));
        return arguments.length === 1
          ? new NativeWebSocket(nextUrl)
          : new NativeWebSocket(nextUrl, protocols);
      }
      NoatinWorkPreviewAppWebSocket.prototype = NativeWebSocket.prototype;
      Object.setPrototypeOf(NoatinWorkPreviewAppWebSocket, NativeWebSocket);
      Object.defineProperty(NoatinWorkPreviewAppWebSocket, 'name', { value: 'WebSocket' });
      window.WebSocket = NoatinWorkPreviewAppWebSocket;
    }
  };
  ...
  const sendHover = (event) => {
    ...
    pendingHover = event;
    if (window.__noatinworkPreviewHoverFrame) return;
    window.__noatinworkPreviewHoverFrame = window.requestAnimationFrame(() => {
      window.__noatinworkPreviewHoverFrame = 0;
  ...
  window.addEventListener('message', (event) => {
    ...
    if (!data || data.source !== 'noatinwork-preview-parent' || data.version !== VERSION) return;
...
export const normalizeProxyTargetUrl = (rawUrl, { allowExternal = false } = {}) => {
  ...
  // Only keep origin here; the proxy path is preserved on the NoatinWork side.
...
const appendProxyAuthToProxyUrl = (value, { previewToken = '', urlAuthToken = '' } = {}) => {
  ...
  try {
    const parsed = new URL(value, 'http://noatinwork-preview.local');
...
export const createPreviewProxyRuntime = ({
  ...
  const attach = (app, {
    ...
    const injectPreviewBridge = (bodyText, targetOrigin, bridgeNonce) => {
      ...
      const targetOriginScript = `<script${nonceAttr}>window.__noatinworkPreviewTargetOrigin=${JSON.stringify(targetOrigin || '')};</script>`;
    ...
    const proxy = createProxyMiddleware({
      ...
      on: {
        proxyReq: (proxyReq, req) => {
          ...
          // Keep local dev servers from receiving NoatinWork credentials.
          proxyReq.removeHeader('cookie');
          proxyReq.removeHeader('authorization');
          proxyReq.removeHeader('x-noatinwork-ui-session');
        ...
        proxyRes: responseInterceptor(async (responseBuffer, proxyRes, req, res) => {
          ...
          // Allow the dev server response to be framed inside NoatinWork even

...
const requestChatForceScrollBottom = (sessionId: string): void => {
  ...
  window.dispatchEvent(new CustomEvent('noatinwork:chat-force-scroll-bottom', {
    detail: { sessionId },
  }));
};
...
const createOrReuseReviewSession = async (originalSessionID: string, directory: string, expectedRuntimeKey?: string): Promise<Session> => {
  ...
  if (existingReviewID) {
    ...
    await patchSessionMetadata(originalSessionID, directory, (metadata) => {
      const next = { ...metadata };
      const noatinwork = next.noatinwork;
      if (noatinwork && typeof noatinwork === 'object' && !Array.isArray(noatinwork)) {
        const rest = { ...(noatinwork as Record<string, unknown>) };
        delete rest.reviewSessionID;
        next.noatinwork = rest;
      }
      return next;
    });
  }

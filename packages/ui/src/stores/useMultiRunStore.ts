...
import { getWorktreeSetupWaitEnabled, saveWorktreeSetupCommands } from '@/lib/noatinworkConfig';
...
const registerCreatedSession = (session: Session, directory: string): Session => {
  ...
  useSessionUIStore.getState().markSessionAsNoatinWorkCreated(session.id);

...
import {
  ...
  commands,
  discoverNoatinWorkInstanceOnPort,
...
async function withTempNoatinWorkDataDir(fn) {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'noatinwork-cli-test-'));
...
async function startMockNoatinWorkServer(options = {}) {
...
function spawnNoatinWorkLikeIdleProcess() {
  return spawn(process.execPath, ['-e', 'setInterval(() => {}, 1000)', 'noatinwork-idle'], { stdio: 'ignore' });
}

function spawnNoatinWorkLikeHungServer(port) {
  const script = `
    ...
    setInterval(() => {}, 1000);
  `;
  return spawn(process.execPath, ['-e', script, 'noatinwork-hung-server'], { stdio: 'ignore' });
}
...
describe('cli args', () => {
  ...
  it('parses explicit connect-url server overrides', () => {
    const parsed = parseArgs(['connect-url', '--server', 'https://noatinwork.example.com', '--port', '3002']);

    expect(parsed.command).toBe('connect-url');
    expect(parsed.options.server).toBe('https://noatinwork.example.com');
  ...
  it('parses session create options', () => {
    ...
    expect(parsed.options.branch).toBe('noatinwork/side-task');
  ...
  it('builds session create payloads from CLI options', () => {
    expect(buildSessionCreatePayload({
      ...
      worktree: {
        name: 'side-task',
        branchName: 'noatinwork/side-task',
        startRef: 'main',
      },
      prompt: 'Investigate cache invalidation',
      model: 'openai/gpt-5.5',
  ...
  it('formats projects compactly', () => {
    expect(formatProjectLine({
      id: 'path_repo',
      label: 'NoatinWork',
      path: '/repo/noatinwork',
    })).toBe('- `NoatinWork` — `path_repo` — `/repo/noatinwork`');
  });
...
describe('cli API target resolution', () => {
  ...
  it('fails when multiple non-default instances are running', async () => {
    await expect(resolveTargetPort({}, {
      ...
      isServerHealthReady: async () => false,
    })).rejects.toThrow('Multiple NoatinWork instances are running');
  });
});
...
describe('network-exposed auth validation', () => {
  ...
  it('allows explicit unsafe LAN override from process env only', () => {
    const previous = process.env.NOATINWORK_ALLOW_UNAUTHENTICATED_LAN;
    process.env.NOATINWORK_ALLOW_UNAUTHENTICATED_LAN = 'true';
    try {
      expect(() => assertAuthenticatedNetworkExposure({ host: '0.0.0.0' })).not.toThrow();
    } finally {
      if (typeof previous === 'string') {
        process.env.NOATINWORK_ALLOW_UNAUTHENTICATED_LAN = previous;
      } else {
        delete process.env.NOATINWORK_ALLOW_UNAUTHENTICATED_LAN;
      }
    }
  });
});
...
describe('serve host resolution', () => {
  it('uses NOATINWORK_HOST when --host is not provided', () => {
    const previous = process.env.NOATINWORK_HOST;
    process.env.NOATINWORK_HOST = '192.0.2.20';
    try {
      expect(resolveServeHost(undefined)).toBe('192.0.2.20');
    } finally {
      if (typeof previous === 'string') {
        process.env.NOATINWORK_HOST = previous;
      } else {
        delete process.env.NOATINWORK_HOST;
      }
    }
  });

  it('prefers explicit --host over NOATINWORK_HOST', () => {
    const previous = process.env.NOATINWORK_HOST;
    process.env.NOATINWORK_HOST = '192.0.2.20';
    try {
...
describe('CLI HTTP helpers', () => {
  it('sends one typed request to the shared control endpoint', async () => {
    ...
    globalThis.fetch = async (url, options = {}) => {
      expect(new URL(String(url)).pathname).toBe('/api/noatinwork/control');
  ...
  it('retries UI-authenticated API requests with the stored instance password', async () => {
    await withTempNoatinWorkDataDir(async () => {
      ...
      try {
        const { response, body } = await requestJson(port, '/api/noatinwork/tunnel/start', {
          method: 'POST',
  ...
  it('prefers the stored instance password over a non-explicit env password', async () => {
    await withTempNoatinWorkDataDir(async () => {
      ...
      try {
        const { response, body } = await requestJson(port, '/api/noatinwork/scheduled-tasks/status', {
  ...
  it('authenticates desktop-local API requests with the stored client token', async () => {
    await withTempNoatinWorkDataDir(async (dir) => {
      const port = 57123;
...
describe('cli entry detection', () => {
  const modulePath = '/tmp/noatinwork/bin/cli.js';
  const moduleUrl = pathToFileURL(modulePath).href;

  it('resolves symlinked entry paths before comparing', () => {
    const symlinkPath = '/usr/local/bin/noatinwork';
  ...
  it('accepts wrapper binary name fallback when requested', () => {
    const wrapperPath = '/home/user/.local/bin/noatinwork';
    expect(isModuleCliExecution(wrapperPath, moduleUrl, undefined, 'noatinwork')).toBe(true);
  });
...
describe('isNoatinWorkCmdline', () => {
  it('accepts NoatinWork CLI and daemon cmdlines', () => {
    expect(isNoatinWorkCmdline('node /x/@noatinwork-app/web/bin/cli.js serve')).toBe(true);
    expect(isNoatinWorkCmdline('node /x/@noatinwork-app/web/server/index.js --port 9090')).toBe(true);
    expect(isNoatinWorkCmdline('bun /home/u/projects/noatinwork/packages/web/server/index.js --port 3001')).toBe(true);
  });

  it('rejects recycled and unrelated processes (issue #1721)', () => {
    expect(isNoatinWorkCmdline('node /home/herjarsa/npm-global/bin/agentmemory')).toBe(false);
    expect(isNoatinWorkCmdline('node /usr/lib/node_modules/npm/bin/npm-cli.js install')).toBe(false);
    expect(isNoatinWorkCmdline('')).toBe(false);
    expect(isNoatinWorkCmdline(null)).toBe(false);
  });
});

describe('isNoatinWorkProcessRunning', () => {
  it('returns false for a dead PID', () => {
    expect(isNoatinWorkProcessRunning(2147483646)).toBe(false);
  });
  ...
  it.skipIf(process.platform !== 'linux' && process.platform !== 'darwin')(
    'returns false for a live non-NoatinWork PID',
    async () => {
      const child = spawn('sleep', ['30'], { stdio: 'ignore' });
      try {
        await new Promise((resolve) => setTimeout(resolve, 150));
        expect(isNoatinWorkProcessRunning(child.pid)).toBe(false);
      } finally {
        child.kill('SIGKILL');
      }
    }
...
describe('lifecycle instance discovery', () => {
  it('does not attribute a desktop runtime response to a different explicit port', async () => {
    await withTempNoatinWorkDataDir(async (dir) => {
      ...
      const instance = await discoverNoatinWorkInstanceOnPort(3003, {
  ...
  it('attributes a desktop runtime response to its configured desktop port', async () => {
    await withTempNoatinWorkDataDir(async (dir) => {
      ...
      const instance = await discoverNoatinWorkInstanceOnPort(57123, {
        fetchImpl: async () => createMockJsonResponse({ runtime: 'desktop', pid: 934 }),
      });
  ...
  it('keeps pid and instance files when live port probe confirms a cmdline mismatch', async () => {
    await withTempNoatinWorkDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        ...
        getNoatinWorkProcessState: () => 'mismatched',
      });
  ...
  it('preserves matched pid and instance files when the recorded port probe is inconclusive', async () => {
    await withTempNoatinWorkDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        fetchImpl: async () => createMockJsonResponse(null, false),
        getNoatinWorkProcessState: () => 'matched',
      });
  ...
  it('preserves unknown-identity pid and instance files when the recorded port probe is inconclusive', async () => {
    await withTempNoatinWorkDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        fetchImpl: async () => createMockJsonResponse(null, false),
        getNoatinWorkProcessState: () => 'unknown',
      });
  ...
  it('uses the live system-info pid instead of a stale NoatinWork-looking pid-file pid', async () => {
    await withTempNoatinWorkDataDir(async () => {
      const port = 45127;
  ...
  it('discovers an explicit live NoatinWork port without a pid-file registry entry', async () => {
    await withTempNoatinWorkDataDir(async () => {
      const port = 45125;
      const instances = await discoverLifecycleInstances(
  ...
  it('cleans a matched pid-file entry without stopping it when the recorded port is free', async () => {
    await withTempNoatinWorkDataDir(async () => {
      const port = await allocateLoopbackPort();
      const child = spawnNoatinWorkLikeIdleProcess();
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
        await new Promise((resolve) => setTimeout(resolve, 150));
...
describe('lifecycle commands with unmanaged explicit ports', () => {
  it('serve refuses to start on a live NoatinWork port without requiring pid files', async () => {
    await withTempNoatinWorkDataDir(async () => {
      const server = await startMockNoatinWorkServer();
      try {
  ...
  it('stop --port can recover a matched pid-file instance whose HTTP endpoint is unresponsive', async () => {
    await withTempNoatinWorkDataDir(async () => {
      const port = await allocateLoopbackPort();
      const child = spawnNoatinWorkLikeHungServer(port);
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
  ...
  it('plain stop ignores a stale CLI registry entry that resolves to desktop runtime', async () => {
    await withTempNoatinWorkDataDir(async () => {
      const server = await startMockNoatinWorkServer({ runtime: 'desktop' });
      const child = spawn('sleep', ['30'], { stdio: 'ignore' });

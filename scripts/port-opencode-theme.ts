...
/*
 * Port OpenCode themes into full NoatinWork theme JSON files.
 *
 * Usage:
 *   bun run themes:port:opencode --list
 *   bun run themes:port:opencode github cursor lucent-orng --force
 *   bun run themes:port:opencode aura --out-dir /tmp/noatinwork-theme-port-test --force
 ...
 *     or existing NoatinWork theme JSON files
 *   - writes complete NoatinWork light/dark theme JSON output with solid core
...
type ThemeSource = DesktopTheme | ContextThemeJson | NoatinWorkTheme;
...
function isNoatinWorkTheme(value: ThemeSource): value is NoatinWorkTheme {
...
function syntheticTokensFromNoatinWorkTheme(theme: NoatinWorkTheme): ResolvedTheme {
...
async function main(): Promise<void> {
  ...
  const generated: Array<{ fileName: string; theme: NoatinWorkTheme }> = [];
    ...
    if (isNoatinWorkTheme(source)) {
      ...
      generated.push({
        ...
        theme: buildTheme(
          syntheticTheme,
          variant,
          syntheticTokensFromNoatinWorkTheme(source),

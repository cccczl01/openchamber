...
interface Item {
  name: string;
  source: 'noatinwork' | 'opencode' | 'skill';
  description?: string;
...
describe('mergeCommandAutocompleteItems', () => {
  ...
  test('deduplicates every pairwise source collision by executable precedence', () => {
    const builtIn: Item = { name: 'compact', source: 'noatinwork', isBuiltIn: true };
  ...
  test('keeps a case-distinct command when the built-in is disabled', () => {
    const builtIn: Item = { name: 'init', source: 'noatinwork', isBuiltIn: true };
  ...
  test('keeps first-seen ordering and unrelated commands', () => {
    const builtIns: Item[] = [{ name: 'undo', source: 'noatinwork' }];
    const commands: Item[] = [

...
export type DesktopSettings = {
  ...
  // User-added skills catalogs (persisted to ~/.config/noatinwork/settings.json)
...
const getElectronRuntime = (): ElectronRuntimeGlobal | null => {
  ...
  return (window as unknown as { __NOATINWORK_ELECTRON__?: ElectronRuntimeGlobal }).__NOATINWORK_ELECTRON__ ?? null;
};

const getDesktopBridge = (): DesktopBridgeGlobal | null => {
  if (typeof window === 'undefined') return null;
  return (window as unknown as { __NOATINWORK_DESKTOP__?: DesktopBridgeGlobal }).__NOATINWORK_DESKTOP__ ?? null;
};
...
export const isDesktopLocalOriginActive = (): boolean => {
  ...
  const local = typeof window.__NOATINWORK_LOCAL_ORIGIN__ === 'string' ? window.__NOATINWORK_LOCAL_ORIGIN__ : '';
...
export const getDesktopHomeDirectory = async (): Promise<string | null> => {
  if (typeof window !== 'undefined') {
    const embedded = window.__NOATINWORK_HOME__;
...
export const downloadDesktopUpdate = async (
  ...
  try {
    if (typeof onProgress === 'function' && bridge?.listen) {
      unlisten = await bridge.listen('noatinwork:update-progress', (evt) => {

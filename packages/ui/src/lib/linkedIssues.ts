...
export const getLinkedIssues = (session: Session | null | undefined): LinkedIssue[] => {
  ...
  if (!isRecord(noatinwork) || !Array.isArray(noatinwork.linked_issues)) return [];
  // Malformed entries are dropped rather than rendered: a half-written link
  // has no row worth showing.
  return noatinwork.linked_issues.filter(isLinkedIssue);
};
...
export const withLinkedIssue = (
  ...
  const current = Array.isArray(noatinwork.linked_issues)
    ? noatinwork.linked_issues.filter(isLinkedIssue)
    : [];

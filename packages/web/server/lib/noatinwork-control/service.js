...
import { NoatinWorkControlError, asControlError } from './error.js';
import { NOATINWORK_CONTROL_ACTIONS } from './actions.js';

const DEFAULT_WAIT_TIMEOUT_SECONDS = 600;
const MAX_WAIT_TIMEOUT_SECONDS = 86_400;
const WAIT_POLL_INTERVAL_MS = 500;
const CONTROL_ACTIONS = new Set(NOATINWORK_CONTROL_ACTIONS);
...
const positiveInteger = (value, fallback, field) => {
  ...
  if (!Number.isSafeInteger(number) || number < 1) {
    throw new NoatinWorkControlError(`${field} must be a positive integer`, 400);
  }
  return number;
};
...
const normalizeWaitTimeoutMs = (value) => {
  ...
  if (!Number.isSafeInteger(seconds) || seconds < 1 || seconds > MAX_WAIT_TIMEOUT_SECONDS) {
    throw new NoatinWorkControlError(`timeout must be from 1 to ${MAX_WAIT_TIMEOUT_SECONDS} seconds`, 400);
  }
  return seconds * 1000;
};
...
const parseModel = (value) => {
  ...
  if (!model) throw new NoatinWorkControlError('model is required', 400);
  const slashIndex = model.indexOf('/');
  if (slashIndex <= 0 || slashIndex === model.length - 1) {
    throw new NoatinWorkControlError('model must be in provider/model format', 400);
  }
...
const parseWeekdays = (value) => {
  ...
  if (!raw) throw new NoatinWorkControlError('weekly is required', 400);
  ...
  if (weekdays.some((entry) => !Number.isInteger(entry) || entry < 0 || entry > 6)) {
    throw new NoatinWorkControlError('weekly must contain weekdays from 0 to 6', 400);
  }
...
const buildSchedule = (input) => {
  ...
  if (selectors.length !== 1) {
    throw new NoatinWorkControlError('Provide exactly one of daily, weekly, once, or cron', 400);
  }
  ...
  if (weekly) {
    ...
    if (!time) throw new NoatinWorkControlError('time is required with weekly', 400);
    return { kind: 'weekly', weekdays: parseWeekdays(weekly), times: [time], ...(timezone ? { timezone } : {}) };
  }
  if (once) {
    const time = asNonEmptyString(input.time);
    if (!time) throw new NoatinWorkControlError('time is required with once', 400);
...
const buildScheduledTask = (input) => {
  ...
  if (!name) throw new NoatinWorkControlError('name is required', 400);
  if (!prompt) throw new NoatinWorkControlError('prompt is required', 400);
  const model = parseModel(input.model);
  const goalTokenBudget = input.goalTokenBudget;
  if (goalTokenBudget !== undefined && input.goal !== true) {
    throw new NoatinWorkControlError('goalTokenBudget requires goal', 400);
  }
  if (goalTokenBudget !== undefined && (!Number.isSafeInteger(goalTokenBudget) || goalTokenBudget < 1000 || goalTokenBudget > 100_000_000)) {
    throw new NoatinWorkControlError('goalTokenBudget must be from 1000 to 100000000', 400);
  }
  return {
    name,
...
export const createNoatinWorkControlService = (dependencies) => {
  const {
  ...
  const wait = (duration, signal) => {
    ...
    if (signal.aborted) return Promise.reject(new NoatinWorkControlError('NoatinWork action was cancelled', 499));
    return new Promise((resolve, reject) => {
      const onAbort = () => {
        signal.removeEventListener('abort', onAbort);
        reject(new NoatinWorkControlError('NoatinWork action was cancelled', 499));
      };
  ...
  const sessionStatus = async (client, sessionID, directory) => {
    ...
    if (!statuses || typeof statuses !== 'object' || Array.isArray(statuses)) {
      throw new NoatinWorkControlError('Invalid session status response', 500);
    }
  ...
  const waitForIdle = async ({ client, sessionID, directory, timeoutMs, requireActivity, baselineMessageID, startedAt, signal }) => {
    ...
    while (true) {
      if (signal?.aborted) throw new NoatinWorkControlError('NoatinWork action was cancelled', 499);
      ...
      if (remaining <= 0) {
        throw new NoatinWorkControlError(`Session did not become idle within ${Math.ceil(timeoutMs / 1000)} seconds`, 500);
      }
  ...
  const executeSessionAction = async (action, input, contextDirectory, signal) => {
    if (input.timeout !== undefined && input.wait !== true) throw new NoatinWorkControlError('timeout requires wait', 400);
    if (input.lastAssistant === true && input.wait !== true) throw new NoatinWorkControlError('lastAssistant requires wait', 400);
    ...
    if (action === 'session.create') {
      ...
      if (!sessionID) throw new NoatinWorkControlError('sessionId is required', 400);
  ...
  const execute = async (action, input = {}, contextDirectory, options = {}) => {
    try {
      if (!CONTROL_ACTIONS.has(action)) {
        throw new NoatinWorkControlError(`Unsupported NoatinWork action: ${action || 'missing'}`, 400);
      }
      ...
      if (action.startsWith('schedule.')) {
        ...
        if (SCHEDULE_TASK_ID_ACTIONS.has(action) && !taskID) {
          throw new NoatinWorkControlError('taskId is required', 400);
        }
        ...
        switch (action) {
          ...
          case 'schedule.toggle': {
            if (typeof input.disabled !== 'boolean') {
              throw new NoatinWorkControlError('disabled is required for schedule.toggle', 400);
            }
      ...
      if (action.startsWith('session.')) {
        ...
        if (!sessionID) throw new NoatinWorkControlError('sessionId is required', 400);
        if (!directory) throw new NoatinWorkControlError('directory is required', 400);
        ...
        if (action === 'session.messages') {
          ...
          if (!['all', 'user', 'assistant'].includes(role)) throw new NoatinWorkControlError('role must be all, user, or assistant', 400);
          const last = input.last === true || input.lastAssistant === true;
          if (input.all === true && (last || input.limit !== undefined)) throw new NoatinWorkControlError('all cannot be combined with last or limit', 400);
          if (last && input.limit !== undefined) throw new NoatinWorkControlError('last cannot be combined with limit', 400);
      ...
      throw asControlError(error, `Failed to execute ${action || 'NoatinWork action'}`);
    }
  };

...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __NOATINWORK_VSCODE_THEME__?: Theme }).__NOATINWORK_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('noatinwork:theme-hmr', handleThemeHmr);
    return () => window.removeEventListener('noatinwork:theme-hmr', handleThemeHmr);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('noatinwork:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('noatinwork:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __noatinworkApplyThemeSync?: (payload: ThemeSyncPayload) => void;
    };

    scopedWindow.__noatinworkApplyThemeSync = applyIncomingThemeSync;

    if (receivesParentThemeSync && window.parent !== window) {
      window.parent.postMessage({ type: 'noatinwork:theme-sync-request' }, window.location.origin);
    }

    return () => {
      if (scopedWindow.__noatinworkApplyThemeSync === applyIncomingThemeSync) {
        delete scopedWindow.__noatinworkApplyThemeSync;
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'noatinwork:theme-sync' || !data.payload) {
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('noatinwork:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('noatinwork:settings-synced', handleSettingsSynced);
  }, [receivesParentThemeSync]);

# NoatinWork Control Service

## Purpose

This module owns the typed control contract shared by the NoatinWork CLI and
the managed OpenCode `noatinwork` tool. Both adapters delegate to
`createNoatinWorkControlService()`; neither adapter may call or spawn the
other.
...
- `service.js` validates and executes the fixed project, model, session, and
  ...
  the filtered `NOATINWORK_AGENT_TOOL_*` exports. `schedule.toggle` requires
...
- `../agent-tool/runtime.js` is the managed-tool adapter. It wraps service
  ...
  loopback credential.
- `../noatinwork-sessions/routes.js` and `../scheduled-tasks/service.js` own

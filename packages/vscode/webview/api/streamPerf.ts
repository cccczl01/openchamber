...
declare global {
  interface Window {
    __noatinworkVsCodeStreamPerfState__?: StreamPerfState;
  }
}
...
const ensurePerfState = (): StreamPerfState | null => {
  ...
  if (!window.__noatinworkVsCodeStreamPerfState__) {
    const startedAt = Date.now();
    window.__noatinworkVsCodeStreamPerfState__ = {
  ...
  return window.__noatinworkVsCodeStreamPerfState__;
};

...
const normalizeNoatinWorkSessionStatus = (payload: Event): Event | null => {
  ...
  if (record.type !== "noatinwork:session-status") return null
  ...
  return {
    id: typeof record.id === "string" && record.id.length > 0
      ? record.id
      : `noatinwork-status-${sessionID}-${Date.now()}`,
    type: "session.status",
    properties: {
      sessionID,
      status,
...
const normalizeEventType = (payload: Event): Event => {
  const normalizedNoatinWorkStatus = normalizeNoatinWorkSessionStatus(payload)
  if (normalizedNoatinWorkStatus) {
    return normalizedNoatinWorkStatus
  }
...
export function createEventPipeline(input: EventPipelineInput): EventPipeline {
  ...
  if (typeof globalThis.window !== "undefined") {
    globalThis.window.addEventListener("noatinwork:system-resume", onSystemResume)
  ...
  const cleanup = () => {
    ...
    if (typeof globalThis.window !== "undefined") {
      globalThis.window.removeEventListener("noatinwork:system-resume", onSystemResume)

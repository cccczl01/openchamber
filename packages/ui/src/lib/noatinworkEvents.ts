...
type NoatinWorkEvent = ScheduledTaskRanEvent | SessionCreatedEvent;
type Listener = (event: NoatinWorkEvent) => void;
...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'noatinwork:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type === 'noatinwork:heartbeat') {
    return;
  }

  if (envelope.type === 'noatinwork:session-created') {
  ...
  if (envelope.type !== 'noatinwork:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const source = new EventSource(getRuntimeUrlResolver().sse('/api/noatinwork/events'));
  source.onopen = () => {
    resetHeartbeatTimer();
  };
...
export const subscribeNoatinWorkEvents = (listener: Listener): (() => void) => {

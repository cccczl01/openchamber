...
const { registerNoatinWorkRoutes } = await import('./noatinwork-routes.js');
...
const createApp = ({ environment = {}, storedOptions = {} } = {}) => {
  ...
  const dependencies = {
    ...
    __dirname: '/opt/noatinwork/server',
    noatinworkDataDir: '/tmp/noatinwork',
  ...
  registerNoatinWorkRoutes(app, dependencies);
...
beforeEach(() => {
  ...
  packageManager.getUpdateCommand.mockReturnValue('npm install -g @noatinwork-app/web@latest');
});
...
describe('NoatinWork foreground update route', () => {
  ...
  it('rejects an unsafe systemd unit override before starting an update job', async () => {
    const { app } = createApp({
      environment: {
        ...
        NOATINWORK_SYSTEMD_UNIT: 'noatinwork.service; rm -rf /',
      },
    });
  ...
  it('queues the install in a transient systemd unit and returns its job identifier', async () => {
    ...
    const { app } = createApp({
      environment: {
        INVOCATION_ID: 'systemd-invocation',
        NOATINWORK_SYSTEMD_UNIT: 'noatinwork@wsl.service',
    ...
    await request(app)
      ...
      .expect(200, {
        ...
        jobId: 'noatinwork-update-1700000000000',
        logPath: 'journalctl --user-unit noatinwork-update-1700000000000.service',
      });

    expect(childProcess.spawnSync).toHaveBeenCalledWith('systemd-run', [
      '--user',
      '--unit=noatinwork-update-1700000000000',
      '--collect',
      '--service-type=exec',
      '--setenv=PATH=/home/syu/.npm-global/bin:/usr/bin:/bin',
      '/bin/sh',
      '-c',
      "set -eu\nnpm install -g @noatinwork-app/web@latest\nsystemctl --user restart 'noatinwork@wsl.service'",
    ], {
      encoding: 'utf8',

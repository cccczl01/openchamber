...
const useExternalLinkInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (anchor.getAttribute('data-noatinwork-file-link') === 'true') {
        return;
      }
...
const FILE_LINK_SELECTOR = '[data-noatinwork-file-link="true"]';
const BLOCK_PATH_TOKEN_ATTR = 'data-noatinwork-block-path-token';
const BLOCK_PATH_TOKEN_SELECTOR = `[${BLOCK_PATH_TOKEN_ATTR}]`;
const CODE_BLOCK_PATH_SCANNED_ATTR = 'data-noatinwork-block-paths-scanned';
...
// `data-noatinwork-block-path-token`. `annotateFileLinks` then promotes those
...
// `data-noatinwork-block-paths-scanned` once processed so the walk is not
...
const useFileReferenceInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const clearFileLinkAttributes = (candidate: HTMLElement) => {
      candidate.removeAttribute('data-noatinwork-file-link');
      candidate.removeAttribute('data-noatinwork-file-ref');
      candidate.removeAttribute('data-noatinwork-file-path');
    ...
    const annotateFileLinks = () => {
      ...
      for (const candidate of Array.from(candidates)) {
        ...
        void existsPromise.then((exists) => {
          ...
          candidate.setAttribute('data-noatinwork-file-link', 'true');
          candidate.setAttribute('data-noatinwork-file-ref', latestRawCandidate);
          candidate.setAttribute('data-noatinwork-file-path', latestResolved.resolvedPath);
    ...
    const openFileReference = async (sourceElement: HTMLElement) => {
      const raw = sourceElement.getAttribute('data-noatinwork-file-ref') || extractPathCandidateFromElement(sourceElement);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target;
      if (!(target instanceof HTMLElement) || target.getAttribute('data-noatinwork-file-link') !== 'true') {
        return;
      }

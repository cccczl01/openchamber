...
export function getWebviewHtml(options: WebviewHtmlOptions): string {
  ...
  // 
  // Logo geometry matches NoatinWorkLogo.tsx:
...
<head>
  ...
  <title>NoatinWork</title>
</head>
<body>
  <!-- Initial loading screen with simplified NoatinWork logo -->
  ...
  <script>
    ...
    window.__NOATINWORK_HOME__ = "${workspaceFolder.replace(/\\/g, '\\\\')}";
    
    function getBootstrapMessages() {
      var locale = 'en';
      try {
        var rawLocale = window.localStorage.getItem('noatinwork.i18n.v1');
        if (rawLocale) {
  ...
  <script type="module">
    ...
    if (!devServerUrl) {
      ...
      const getDevMessages = () => {
        try {
          const rawLocale = window.localStorage.getItem('noatinwork.i18n.v1');
          if (rawLocale) {
      ...
      const tryLoadDevBundle = () => {
        ...
        Promise.resolve()
          ...
          .catch((error) => {
            attempt += 1;
            console.warn('[NoatinWork] VS Code webview dev bundle unavailable, retrying...', error);

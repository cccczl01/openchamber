# Managed NoatinWork Agent Tool

## Purpose

This module exposes NoatinWork orchestration to agents as one typed OpenCode
custom tool named `noatinwork`. It is injected only when NoatinWork launches
...
1. The NoatinWork HTTP listener binds and publishes its authoritative port.
2. `prepareManagedOpenCodeEnv()` materializes the plugin under
   `<noatinwork-data-dir>/agent-tool/` and appends its `file://` URL to
...
4. The plugin calls `POST /api/noatinwork/agent-tool` with its typed input and
   OpenCode's authoritative session directory.
5. The route delegates the fixed action allowlist directly to the shared
   NoatinWork control service. The CLI uses the same service through its
...
- The tool exposes only agent-relevant actions
  (`NOATINWORK_AGENT_TOOL_ACTIONS`): `schedule.status` stays CLI-only because
...
- External OpenCode selected with `OPENCODE_HOST` or skip-start: not injected,
  because NoatinWork does not control that process environment.

...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  const options = {
    port: DEFAULT_PORT,
    host: undefined,
    uiPassword: process.env.NOATINWORK_UI_PASSWORD || undefined,
    json: false,
    all: false,
    follow: true,
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (name) {
      ...
      case 'd':
        ...
        break;
      case 'try-cf-tunnel':
        removedFlagErrors.push('`--try-cf-tunnel` was removed. Use: noatinwork tunnel start --provider cloudflare --mode quick');
        break;
      case 'tunnel-qr':
        removedFlagErrors.push('`--tunnel-qr` was removed. Use: noatinwork tunnel start ... --qr');
        break;
      ...
      case 'tunnel':
        removedFlagErrors.push(`\`--${name}\` was removed from top-level serve flow. Use: noatinwork tunnel start ...`);
        break;
      default:
...
function showHelp() {
  console.log(`
 NoatinWork - Web interface for the OpenCode AI coding agent

USAGE:
  noatinwork [COMMAND] [OPTIONS]
...
ENVIRONMENT:
  NOATINWORK_HOST             Bind address (e.g. 0.0.0.0 for all interfaces)
  NOATINWORK_UI_PASSWORD      Alternative to --ui-password flag
  NOATINWORK_API_ONLY         Set to true/1 to start API routes only
  NOATINWORK_DATA_DIR         Override NoatinWork data directory
  ...
  NOATINWORK_OPENCODE_HOSTNAME  Bind hostname for managed OpenCode server (default: 127.0.0.1)

EXAMPLES:
  noatinwork                    # Start in daemon mode on default port 3000 (or free port)
  noatinwork --port 8080        # Start on port 8080 (daemon)
  noatinwork --lan --port 3002  # Start on LAN at 0.0.0.0:3002
  noatinwork serve --foreground # Start in foreground (for systemd Type=simple)
  noatinwork connect-url --port 3000 --qr
  noatinwork connect-url --server https://noatinwork.example.com
  noatinwork control           # Show control-plane commands for agents/scripts
  noatinwork startup enable     # Start NoatinWork at user login
  noatinwork tunnel help        # Show tunnel lifecycle help
  noatinwork logs               # Follow logs for latest running instance
`);
}

function showControlHelp() {
  console.log(`
 NoatinWork Control Commands

USAGE:
  noatinwork <COMMAND> [OPTIONS]

COMMANDS:
  status                         Show running NoatinWork runtimes
...
DETAILED HELP:
  noatinwork session --help     Show session creation, status, and message options
  noatinwork models --help      Show model defaults and favorites help
  noatinwork projects --help    Show project list help
  noatinwork schedule --help    Show scheduled task actions and schedule options
  noatinwork tunnel help        Show tunnel lifecycle/status commands
  noatinwork status --help      Show runtime status options

COMMON OPTIONS:
  --json                         Output machine-readable JSON
  -q, --quiet                    Print minimal output
  -p, --port <port>              Target a specific NoatinWork runtime
...
EXAMPLES:
  ...
  noatinwork models
  noatinwork projects
  noatinwork session --help
  noatinwork schedule --help
`);
}

function showStartupHelp() {
  console.log(`
 NoatinWork Startup Commands

USAGE:
  noatinwork startup <SUBCOMMAND> [OPTIONS]
...
EXAMPLES:
  noatinwork startup enable
  noatinwork startup enable --port 3000
  noatinwork startup enable --port 3000 --api-only --host 0.0.0.0
  noatinwork startup status --json
`);
}

function showConnectUrlHelp() {
  console.log(`
 NoatinWork Connect URL

USAGE:
  noatinwork connect-url [OPTIONS]

DESCRIPTION:
  Generate an noatinwork:// connection link for adding this server to another
  NoatinWork app. If no server is running on the selected port, it starts one.
...
OPTIONS:
  ...
  --relay                 Also include the end-to-end-encrypted relay transport
                          ...
                          NOATINWORK_RELAY_URL to use a self-hosted relay.
...
EXAMPLES:
  noatinwork connect-url --port 3000 --qr
  noatinwork connect-url --port 3000 --api-only --lan --server http://workstation.local:3000 --qr
  noatinwork connect-url --server https://noatinwork.example.com --name Workstation
  noatinwork connect-url --relay --name "My laptop"
`);
}
...
USAGE:
  noatinwork tunnel <SUBCOMMAND> [OPTIONS]
...
COMMON OPTIONS:
  -p, --port              Target NoatinWork instance port
...
BEHAVIOR NOTES:
  - One active tunnel per NoatinWork instance.
...
PROFILE USAGE:
  noatinwork tunnel profile list [--provider <id>] [--json]
  noatinwork tunnel profile show --name <name> [--provider <id>] [--json]
  noatinwork tunnel profile add --provider <id> --mode managed-remote --name <name> --hostname <host> --token <token> [--force] [--json]
  noatinwork tunnel profile add --provider <id> --mode managed-remote --name <name> --hostname <host> --token-file <path> [--force] [--json]
  noatinwork tunnel profile remove --name <name> [--provider <id>] [--json]

SHELL COMPLETION:
  noatinwork tunnel completion bash   Generate Bash completion script
  noatinwork tunnel completion zsh    Generate Zsh completion script
  noatinwork tunnel completion fish   Generate Fish completion script

EXAMPLES:
  noatinwork tunnel providers
  noatinwork tunnel ready --provider cloudflare
  noatinwork tunnel doctor --provider cloudflare
  noatinwork tunnel status
  noatinwork tunnel start --qr
  noatinwork tunnel start --profile prod-main
  noatinwork tunnel start --provider cloudflare --mode managed-remote --token-file ~/.secrets/cf-token --hostname app.example.com
  noatinwork tunnel start --provider cloudflare --mode managed-local --config ~/.cloudflared/config.yml
  noatinwork tunnel start --dry-run --provider cloudflare --mode managed-remote --token-file ~/.secrets/cf-token --hostname app.example.com
  echo "$TOKEN" | noatinwork tunnel profile add --provider cloudflare --mode managed-remote --name prod-main --hostname app.example.com --token-stdin
  noatinwork tunnel profile list --provider cloudflare
  noatinwork tunnel profile list --json --show-secrets
  noatinwork tunnel stop --port 3000
`);
}
...
function generateCompletionScript(shell) {
  ...
  if (normalized === 'bash') {
    return `# Bash completion for noatinwork tunnel
# Add to ~/.bashrc: eval "$(noatinwork tunnel completion bash)"
_noatinwork_tunnel() {
...
complete -F _noatinwork_tunnel noatinwork
`;
  }

  if (normalized === 'zsh') {
    return `#compdef noatinwork
# Zsh completion for noatinwork tunnel
# Add to ~/.zshrc: eval "$(noatinwork tunnel completion zsh)"
...
compdef _noatinwork noatinwork
`;
  }

  if (normalized === 'fish') {
    return `# Fish completion for noatinwork tunnel
# Save to ~/.config/fish/completions/noatinwork.fish

complete -c noatinwork -n '__fish_use_subcommand' -a 'serve' -d 'Start the web server'
complete -c noatinwork -n '__fish_seen_subcommand_from serve' -l foreground -d 'Run in foreground (for systemd/process managers)'
complete -c noatinwork -n '__fish_seen_subcommand_from serve' -l no-daemon -d 'Run in foreground (alias for --foreground)'
complete -c noatinwork -n '__fish_use_subcommand' -a 'stop' -d 'Stop running instance(s)'
complete -c noatinwork -n '__fish_use_subcommand' -a 'restart' -d 'Stop and start the server'
complete -c noatinwork -n '__fish_use_subcommand' -a 'status' -d 'Show server status'
complete -c noatinwork -n '__fish_use_subcommand' -a 'tunnel' -d 'Tunnel lifecycle commands'
complete -c noatinwork -n '__fish_use_subcommand' -a 'logs' -d 'Tail logs'
complete -c noatinwork -n '__fish_use_subcommand' -a 'update' -d 'Check for updates'

complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'help' -d 'Show tunnel help'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'providers' -d 'Show providers'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'ready' -d 'Check readiness'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'doctor' -d 'Run diagnostics'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'status' -d 'Show tunnel status'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'start' -d 'Start a tunnel'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'stop' -d 'Stop tunnel'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'profile' -d 'Manage profiles'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'completion' -d 'Generate completions'

complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l provider -d 'Provider id'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l mode -d 'Tunnel mode'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l profile -d 'Profile name'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l config -d 'Config path'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l token -d 'Token'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l token-file -d 'Token file path'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l token-stdin -d 'Read token from stdin'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l hostname -d 'Hostname'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l dry-run -d 'Validate without applying'
complete -c noatinwork -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l qr -d 'Show QR code'
`;
  }

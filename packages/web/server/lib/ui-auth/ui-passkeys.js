...
const DEFAULT_RP_NAME = 'NoatinWork';
...
const PASSKEY_STORE_FILE = path.join(NOATINWORK_DATA_DIR, 'ui-passkeys.json');
...
export const createUiPasskeys = ({
  ...
  const beginRegistration = async (req, { label } = {}) => {
    ...
    const options = await generateRegistrationOptions({
      ...
      rpID,
      userID,
      userName: 'noatinwork-ui',
      userDisplayName: 'NoatinWork UI',
  ...
  const finishAuthentication = async (payload) => {
    ...
    if (!passkey) {
      const error = new Error('That passkey is not registered for this NoatinWork instance');
      error.statusCode = 404;
      throw error;
    }

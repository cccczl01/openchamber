...
export const useOpenInAppsStore = create<OpenInAppsState>()((set, get) => ({
  ...
  initialize: () => {
    ...
    window.addEventListener('noatinwork:settings-synced', settingsHandler);
    window.addEventListener('noatinwork:app-ready', appReadyHandler);
    window.addEventListener('noatinwork:installed-apps-updated', updateHandler);

    const appReady = (window as unknown as { __noatinworkAppReady?: boolean }).__noatinworkAppReady;
    if (appReady) {
      void loadInstalledApps();
    }

...
export const registerServerStatusRoutes = (app, dependencies) => {
  const {
    ...
    gracefulShutdown,
    getHealthSnapshot,
    // Port this NoatinWork instance serves on and the tunnel public URL (if
  ...
  const isDevShutdownAllowed = () => {
    ...
    return process.env.NOATINWORK_DEV_SHUTDOWN === 'true';
  };
...
export const registerCommonRequestMiddleware = (app, dependencies) => {
  ...
  app.use((req, res, next) => {
    if (req.path.startsWith('/api/behavior')) {
      ...
      req.path.startsWith('/api/noatinwork/tunnel')
    ) {

...
plugins — this module replicates that mechanism as an NoatinWork runtime API.
...
- `resolve.js` — model selection, mirroring OpenCode's `getSmallModel` chain:
  0. NoatinWork's own settings override (Settings → Sessions → Small Model):
...
- `catalog.js` — models.dev catalog via the shared in-process cache
  ...
  `/api/noatinwork/models-metadata`).

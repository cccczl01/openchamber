...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'noatinwork:files:md-viewer-mode';
  const HTML_VIEWER_MODE_KEY = 'noatinwork:files:html-viewer-mode';
  const JSON_VIEWER_MODE_KEY = 'noatinwork:files:json-viewer-mode';
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('noatinwork:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    return () => {
      window.removeEventListener('noatinwork:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    };
  }, [openPaths]);

#!/usr/bin/env node
/**
 * Fully automated streaming capture for NoatinWork.
 *
 ...
 * prompt through the supported `noatinwork session` CLI, and records until the
...
Records what NoatinWork costs while an assistant response streams in.

Options:
  --url <url>              NoatinWork URL (default: http://localhost:3000)
  --port <port>            NoatinWork CLI port (default: from --url)
...
/**
 * Runs an `noatinwork session` subcommand and returns its parsed JSON.

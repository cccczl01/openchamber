...
- Shared v2 `noatinwork://connect` payload helpers.
...
```text
NOATINWORK_DATA_DIR/client-pairing-sessions.json
```
...
UI can later encode the returned data into noatinwork://connect?v=2&p=...
```
...
```text
noatinwork://connect?v=1&server=...&token=...&label=...
```
...
```text
noatinwork://connect?v=2&p=<base64url-json>
```
...
```text
noatinwork://connect?v=1&server=...&token=...
```
...
Add parse v2 helper.
Use noatinwork://connect?v=2&p=<base64url-json>.
Validate candidates.
...
```text
Existing v1 noatinwork://connect links keep working.

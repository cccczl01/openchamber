...
// Control Center control (iOS 18+): tap the NoatinWork logo to start a new session.
//
...
@available(iOS 18.0, *)
struct NoatinWorkNewSessionControl: ControlWidget {
    var body: some ControlWidgetConfiguration {
        StaticControlConfiguration(kind: "NoatinWorkNewSessionControl") {
        ...
        .description("Start a new NoatinWork session.")
    }
}

@available(iOS 18.0, *)
struct OpenNewSessionIntent: AppIntent {
    static let title: LocalizedStringResource = "New NoatinWork Session"
    static let openAppWhenRun: Bool = true
    static let isDiscoverable: Bool = true

    @MainActor
    func perform() async throws -> some IntentResult & OpensIntent {
        return .result(opensIntent: OpenURLIntent(URL(string: "noatinwork://new")!))
    }
}

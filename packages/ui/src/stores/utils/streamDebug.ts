...
const STREAM_PERF_STORAGE_KEY = 'noatinwork_stream_perf';
...
declare global {
    interface Window {
        __noatinworkStreamPerfState?: StreamPerfState;
        __noatinworkStreamPerformance?: {
            setEnabled: (enabled: boolean) => void;
            reset: () => void;
            getSnapshot: () => StreamPerfSnapshot;
        };
        __noatinworkVsCodeStreamPerfState?: {
            counters: Map<string, PerfCounter>;
            lastReportAt?: number;
            lastUpdatedAt?: number;
...
const ensureStreamPerfState = (): StreamPerfState | null => {
    ...
    if (!window.__noatinworkStreamPerfState) {
        const startedAt = Date.now();
        window.__noatinworkStreamPerfState = {
    ...
    return window.__noatinworkStreamPerfState;
};
...
export const setStreamPerfEnabled = (enabled: boolean): void => {
    ...
    try {
        ...
        delete window.__noatinworkStreamPerfState;
        delete window.__noatinworkVsCodeStreamPerfState;
    } catch {
...
export const resetStreamPerf = (): void => {
    ...
    if (window.__noatinworkVsCodeStreamPerfState) {
        window.__noatinworkVsCodeStreamPerfState = {
            ...window.__noatinworkVsCodeStreamPerfState,
            counters: new Map<string, PerfCounter>(),
            startedAt: Date.now(),
            lastUpdatedAt: Date.now(),
...
export const getStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__noatinworkStreamPerfState;
...
export const getVsCodeStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__noatinworkVsCodeStreamPerfState;
    if (!streamPerfEnabled || !state) {
        return {
            enabled: false,
            startedAt: null,
...
export const streamPerfMark = (metric: string): void => {
    ...
    performance.mark(`noatinwork.${metric}`);
};
...
if (typeof window !== 'undefined') {
    window.__noatinworkStreamPerformance = {

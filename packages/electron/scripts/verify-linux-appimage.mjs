...
export const verifyExtractedPayload = ({
  ...
  const desktopPath = path.join(root, 'noatinwork.desktop');
  ...
  for (const entry of ['Name=NoatinWork', 'Icon=noatinwork', 'StartupWMClass=noatinwork']) {
  ...
  assertElfArchitecture(path.join(root, 'noatinwork'), targetArchitecture, 'Electron executable');
...
const findAppImage = (version, architecture) => {
  ...
  const expected = path.join(electronRoot, 'dist', `NoatinWork-${version}-linux-${suffix}.AppImage`);
...
const main = () => {
  ...
  const target = normalizeTargetArchitecture(process.env.NOATINWORK_TARGET_ARCH || process.arch).node;
  ...
  const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'noatinwork-appimage-'));
  try {

...
export type DesktopSshInstance = {
  ...
  auth: {
    ...
    noatinworkPassword?: DesktopSshStoredSecret;
  };
...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const remoteRaw = isRecord(value.remoteNoatinWork)
    ? value.remoteNoatinWork
    : isRecord(value.remote_noatinwork)
      ? value.remote_noatinwork
      : {};
  ...
  const noatinworkPassword = parseStoredSecret(authRaw.noatinworkPassword || authRaw.noatinwork_password);
  ...
  return {
    ...
    auth: {
      ...
      ...(noatinworkPassword ? { noatinworkPassword } : {}),
    },
    portForwards,
  };
};
...
export const listenDesktopSshStatus = async (
  ...
  const desktop = (window as unknown as { __NOATINWORK_DESKTOP__?: DesktopBridgeGlobal }).__NOATINWORK_DESKTOP__;
  const listen = desktop?.listen;
  if (typeof listen !== 'function') {
    return async () => {};
  }

  const unlisten = await listen('noatinwork:ssh-instance-status', (event) => {

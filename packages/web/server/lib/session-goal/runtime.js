...
// (metadata.noatinwork.goal). While the goal is active, the server keeps the
...
const NOATINWORK_SETTINGS_FILE = path.join(
  process.env.NOATINWORK_DATA_DIR
...
const isSessionGoalEnabled = () => {
  try {
    const raw = fs.readFileSync(NOATINWORK_SETTINGS_FILE, 'utf8');
...
const parseGoalMetadata = (session) => {
  ...
  const namespace = metadata.noatinwork;
...
export const createSessionGoalRuntime = ({
  ...
  const writeGoal = async (sessionId, directory, expectedGoalId, mutate) => {
    ...
    const currentNamespace = currentMetadata.noatinwork && typeof currentMetadata.noatinwork === 'object'
      ? currentMetadata.noatinwork
      : {};
    await openCodeFetch(`/session/${encodeURIComponent(sessionId)}`, {
      ...
      body: {
        metadata: {
          ...currentMetadata,
          noatinwork: { ...currentNamespace, goal: nextGoal },
        },
      },
    });
    return nextGoal;
  ...
  const tick = async (sessionId, directory) => {
    ...
    // TEXT lives under the NoatinWork data dir keyed by session id and is

...
description: Use when creating or modifying NoatinWork shared UI data access, OpenCode SDK calls, `RuntimeAPIs`, runtime fetch/auth/URLs, authenticated browser assets, bridges/proxies, runtime switching, or server API routes.
---
...
- NoatinWork-owned HTTP capabilities use `RuntimeAPIs` where runtime-specific behavior exists, otherwise explicit NoatinWork routes through `runtimeFetch`.
...
| NoatinWork HTTP route | `runtimeFetch('/api/...')` |
...
2. **Keep NoatinWork routes explicit.** Register them before the generic OpenCode proxy.
...
Plain `fetch` is reserved for intentional external origins that are not the active NoatinWork/OpenCode runtime.
...
- NoatinWork routes win before generic proxy fallback.

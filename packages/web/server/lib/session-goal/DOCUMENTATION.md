...
objective stored under `metadata.noatinwork.goal`, with the small model as
an independent progress auditor. Built on NoatinWork's backend-driven
architecture (session-assist is the structural template): the loop lives in
the web server and survives UI disconnects.

## Goal payload (`metadata.noatinwork.goal`)
...
`NOATINWORK_DATA_DIR` or `~/.config/noatinwork`), keyed by the SESSION ID:
...
- `routes.js` — `PUT/GET/DELETE /api/goals/objective/:sessionId`
  (NoatinWork-owned, registered before the generic proxy; JSON parsing via
...
stamp `metadata.noatinwork.goal` onto the fresh session (objective = the
...
`noatinwork session create --prompt <text> --goal` uses the explicit
`POST /api/noatinwork/sessions` orchestration route. The server creates the
...
`noatinwork session send --goal` and `noatinwork session fork --goal` use

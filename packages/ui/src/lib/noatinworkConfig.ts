/**
 * NoatinWork project-level configuration service.
 * Stores per-project settings in ~/.config/noatinwork/<projectId>.json.
 * Migrates from legacy <project>/.noatinwork/noatinwork.json.
 */
...
const CONFIG_FILENAME = 'noatinwork.json';
// LEGACY_PROJECT_CONFIG: legacy per-project config root inside repo.
const LEGACY_CONFIG_DIR = '.noatinwork';
const USER_PROJECTS_DIR_SEGMENTS = ['.config', 'noatinwork', 'projects'];
...
interface NoatinWorkConfig {
  projectPath?: string;
  'setup-worktree'?: string[];
  'setup-worktree-wait'?: boolean;
  projectNotes?: string;
  projectTodos?: NoatinWorkProjectTodoItem[];
  projectPlanFiles?: NoatinWorkProjectPlanFileLink[];
  projectActions?: NoatinWorkProjectAction[];
  projectActionsPrimaryId?: string;
  draftStarters?: DraftStarterRef[];
}

type NoatinWorkProjectActionPlatform = 'macos' | 'linux' | 'windows';

export interface NoatinWorkProjectAction {
  id: string;
  name: string;
  command: string;
  icon?: string | null;
  platforms?: NoatinWorkProjectActionPlatform[];
  autoOpenUrl?: boolean;
  openUrl?: string;
  desktopOpenSshForward?: string;
}

export interface NoatinWorkProjectActionsState {
  actions: NoatinWorkProjectAction[];
  primaryActionId: string | null;
}

export interface NoatinWorkProjectTodoItem {
  id: string;
  text: string;
  completed: boolean;
  createdAt: number;
...
export interface NoatinWorkProjectPlanFileLink {
  id: string;
  path: string;
  createdAt: number;
}

export interface NoatinWorkProjectPlanFile {
  title: string;
  body: string;
  raw: string;
  path: string;
...
export interface NoatinWorkProjectNotesTodos {
  notes: string;
  todos: NoatinWorkProjectTodoItem[];
}

export interface NoatinWorkProjectContextData extends NoatinWorkProjectNotesTodos {
  plans: NoatinWorkProjectPlanFileLink[];
}

export const NOATINWORK_PROJECT_NOTES_MAX_LENGTH = 3000;
export const NOATINWORK_PROJECT_TODO_TEXT_MAX_LENGTH = 120;
const NOATINWORK_PROJECT_ACTION_NAME_MAX_LENGTH = 80;
const NOATINWORK_PROJECT_ACTION_COMMAND_MAX_LENGTH = 4000;
const NOATINWORK_PROJECT_ACTION_OPEN_URL_MAX_LENGTH = 2000;
const NOATINWORK_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH = 300;
const NOATINWORK_PROJECT_PLAN_TITLE_MAX_LENGTH = 160;

const NOATINWORK_ACTION_PLATFORM_SET = new Set<NoatinWorkProjectActionPlatform>(['macos', 'linux', 'windows']);
...
const resolveHomeDirectory = async (): Promise<string | null> => {
  ...
  // In some runtimes, window.__NOATINWORK_HOME__ can be workspace/project-root
...
const sanitizeProjectNotes = (value: unknown): string => {
  ...
  return trimToMaxLength(value, NOATINWORK_PROJECT_NOTES_MAX_LENGTH);
};

const sanitizeProjectTodoItems = (value: unknown): NoatinWorkProjectTodoItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: NoatinWorkProjectTodoItem[] = [];
  for (const entry of value) {
    ...
    const text = trimToMaxLength(textRaw.trim(), NOATINWORK_PROJECT_TODO_TEXT_MAX_LENGTH);
    if (!id || !text) {
      continue;
    }
...
const sanitizeProjectPlanFileLinks = (value: unknown): NoatinWorkProjectPlanFileLink[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: NoatinWorkProjectPlanFileLink[] = [];
...
const sanitizeProjectActionPlatforms = (value: unknown): NoatinWorkProjectActionPlatform[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const unique: NoatinWorkProjectActionPlatform[] = [];
  const seen = new Set<NoatinWorkProjectActionPlatform>();
  for (const entry of value) {
    if (typeof entry !== 'string') {
      continue;
    }
    const normalized = entry.trim().toLowerCase() as NoatinWorkProjectActionPlatform;
    if (!NOATINWORK_ACTION_PLATFORM_SET.has(normalized) || seen.has(normalized)) {
      continue;
    }
    seen.add(normalized);
...
const sanitizeProjectActions = (value: unknown): NoatinWorkProjectAction[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: NoatinWorkProjectAction[] = [];
  const seenIds = new Set<string>();
  ...
  for (const entry of value) {
    ...
    const id = typeof record.id === 'string' ? record.id.trim() : '';
    const name = trimToMaxLength(typeof record.name === 'string' ? record.name.trim() : '', NOATINWORK_PROJECT_ACTION_NAME_MAX_LENGTH);
    const command = trimToMaxLength(typeof record.command === 'string' ? record.command.trim() : '', NOATINWORK_PROJECT_ACTION_COMMAND_MAX_LENGTH);
    ...
    const openUrl = trimToMaxLength(openUrlRaw, NOATINWORK_PROJECT_ACTION_OPEN_URL_MAX_LENGTH);
    ...
    const desktopOpenSshForward = trimToMaxLength(
      ...
      NOATINWORK_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH
    );
...
} | null | undefined): NoatinWorkProjectActionsState => {
...
const sanitizeProjectNotesAndTodos = (value: {
  notes?: unknown;
  todos?: unknown;
} | null | undefined): NoatinWorkProjectNotesTodos => {
  return {
...
const sanitizeProjectContextData = (value: {
  ...
  todos?: unknown;
  plans?: unknown;
} | null | undefined): NoatinWorkProjectContextData => {
...
const sanitizePlanTitle = (value: string): string => {
  return trimToMaxLength(value.trim(), NOATINWORK_PROJECT_PLAN_TITLE_MAX_LENGTH);
};
 ...
 */
async function readNoatinWorkConfig(project: ProjectRef): Promise<NoatinWorkConfig | null> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return null;
  }
  ...
  const parseConfig = (text: string | null): NoatinWorkConfig | null => {
    ...
    try {
      ...
      return parsed as NoatinWorkConfig;
    } catch {
      return null;
    }
  };
  ...
  // 2) Migrate legacy <project>/.noatinwork/noatinwork.json.
  // LEGACY_PROJECT_CONFIG: migrate project-local noatinwork.json -> ~/.config/noatinwork/projects/<projectId>.json
  ...
  try {
    const wrote = await writeNoatinWorkConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyNoatinWorkConfig(projectDirectory);
    }
  } catch {
 ...
 */
async function writeNoatinWorkConfig(
  project: ProjectRef,
  config: NoatinWorkConfig
): Promise<boolean> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return false;
  ...
  try {
    ...
    console.error('Failed to write noatinwork config:', error);
    return false;
  }
}
 ...
 */
async function updateNoatinWorkConfig(
  project: ProjectRef,
  updates: Partial<NoatinWorkConfig>
): Promise<boolean> {
  const existing = await readNoatinWorkConfig(project) || {};
  const merged = { ...existing, ...updates };
  return writeNoatinWorkConfig(project, merged);
}
...
export async function getWorktreeSetupCommands(project: ProjectRef): Promise<string[]> {
  const config = await readNoatinWorkConfig(project);
...
export async function saveWorktreeSetupCommands(project: ProjectRef, commands: string[]): Promise<boolean> {
  ...
  return updateNoatinWorkConfig(project, { 'setup-worktree': filtered });
}
...
export async function saveWorktreeSetupWaitEnabled(project: ProjectRef, enabled: boolean): Promise<boolean> {
  return updateNoatinWorkConfig(project, { 'setup-worktree-wait': enabled });
}
...
export async function saveProjectDraftStarters(project: ProjectRef, starters: DraftStarterRef[]): Promise<boolean> {
  return updateNoatinWorkConfig(project, { draftStarters: sanitizeStarterRefs(starters) });
}

export async function getProjectNotesAndTodos(project: ProjectRef): Promise<NoatinWorkProjectNotesTodos> {
  const config = await readNoatinWorkConfig(project);
...
export async function saveProjectNotesAndTodos(
  project: ProjectRef,
  value: NoatinWorkProjectNotesTodos
): Promise<boolean> {
  ...
  return updateNoatinWorkConfig(project, {
    projectNotes: sanitized.notes,
    projectTodos: sanitized.todos,
  });
}

export async function getProjectContextData(project: ProjectRef): Promise<NoatinWorkProjectContextData> {
  const config = await readNoatinWorkConfig(project);
...
async function getProjectPlanFiles(project: ProjectRef): Promise<NoatinWorkProjectPlanFileLink[]> {
  const config = await readNoatinWorkConfig(project);
...
async function saveProjectPlanFiles(
  project: ProjectRef,
  value: NoatinWorkProjectPlanFileLink[]
): Promise<boolean> {
...
export async function readProjectPlanFile(path: string): Promise<NoatinWorkProjectPlanFile | null> {
...
): Promise<NoatinWorkProjectPlanFileLink | null> {
...
export async function getProjectActionsState(project: ProjectRef): Promise<NoatinWorkProjectActionsState> {
  const config = await readNoatinWorkConfig(project);
...
export async function saveProjectActionsState(
  project: ProjectRef,
  value: NoatinWorkProjectActionsState
): Promise<boolean> {
...
async function deleteLegacyNoatinWorkConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();

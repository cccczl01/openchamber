...
export const DesktopNetworkSettings: React.FC = () => {
  ...
  const isMacDesktop = isLocalDesktop
    ...
    && window.__NOATINWORK_PLATFORM__ === 'darwin';
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(t('settings.noatinwork.desktopNetwork.error.loadFailed'));
        }
        ...
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : t('settings.noatinwork.desktopNetwork.error.loadFailed'));
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const handleLaunchAtLoginToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.noatinwork.desktopNetwork.error.launchAtLoginUnsupported'));
      }
      setLaunchAtLoginEnabled(status.enabled);
    } catch (cause) {
      setLaunchAtLoginEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.noatinwork.desktopNetwork.error.launchAtLoginSaveFailed'));
    } finally {
  ...
  const handleMinimizeToTrayToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status) {
        throw new Error(t('settings.noatinwork.desktopNetwork.error.minimizeToTraySaveFailed'));
      }
      if (!status.supported) {
        throw new Error(t('settings.noatinwork.desktopNetwork.error.minimizeToTrayUnsupported'));
      }
      setMinimizeToTrayEnabled(status.enabled);
    } catch (cause) {
      setMinimizeToTrayEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.noatinwork.desktopNetwork.error.minimizeToTraySaveFailed'));
    } finally {
  ...
  const handleKeepAwakeToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.noatinwork.desktopNetwork.error.keepAwakeUnsupported'));
      }
      setKeepAwakeEnabled(status.enabled);
    } catch (cause) {
      setKeepAwakeEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.noatinwork.desktopNetwork.error.keepAwakeSaveFailed'));
    } finally {
  ...
  const handleSaveAndRestart = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('settings.noatinwork.desktopNetwork.error.saveFailed'));
      }
      ...
      if (!restarted) {
        throw new Error(t('settings.noatinwork.desktopNetwork.error.savedRestartFailed'));
      }
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t('settings.noatinwork.desktopNetwork.error.saveFailed'));
      setIsSaving(false);
    }
  ...
  return (
    <SettingsSection title={t('settings.noatinwork.desktopNetwork.title')}>
      <div className="space-y-3">
          ...
          <div className={SETTINGS_OPTION_STACK_CLASS}>
            {launchAtLoginSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.noatinwork.desktopNetwork.field.launchAtLogin')}
                info={t('settings.noatinwork.desktopNetwork.field.launchAtLoginDescription')}
                ariaLabel={t('settings.noatinwork.desktopNetwork.field.launchAtLoginAria')}
              />
            ) : null}
            ...
            {isMacDesktop ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.noatinwork.desktopNetwork.field.macMenuBar')}
                info={t('settings.noatinwork.desktopNetwork.field.macMenuBarDescription')}
                ariaLabel={t('settings.noatinwork.desktopNetwork.field.macMenuBarAria')}
              />
            ) : null}
            ...
            {minimizeToTraySupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.noatinwork.desktopNetwork.field.minimizeToTray')}
                info={t('settings.noatinwork.desktopNetwork.field.minimizeToTrayDescription')}
                ariaLabel={t('settings.noatinwork.desktopNetwork.field.minimizeToTrayAria')}
              />
            ) : null}
            ...
            {keepAwakeSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.noatinwork.desktopNetwork.field.keepAwake')}
                info={t('settings.noatinwork.desktopNetwork.field.keepAwakeDescription')}
                ariaLabel={t('settings.noatinwork.desktopNetwork.field.keepAwakeAria')}
              />
            ) : null}
          </div>
        ) : null}
        ...
        <SettingsStackedField
          ...
          label={(
            <label htmlFor="desktop-ui-password">
              {t('settings.noatinwork.desktopPassword.field.password')}
            </label>
          )}
          info={t('settings.noatinwork.desktopPassword.field.passwordDescription')}
        >
          <Input
            ...
            placeholder={t('settings.noatinwork.desktopPassword.field.passwordPlaceholder')}
            disabled={isLoading || isSaving}
            required={draftValue}
          ...
          <Button
            ...
            aria-label={t(showPassword ? 'settings.noatinwork.desktopPassword.actions.hidePassword' : 'settings.noatinwork.desktopPassword.actions.showPassword')}
        ...
        <div className={SETTINGS_OPTION_STACK_CLASS}>
          <SettingsCheckboxRow
            ...
            disabled={isLoading || isSaving}
            label={t('settings.noatinwork.desktopNetwork.field.allowLanAccess')}
            info={t('settings.noatinwork.desktopNetwork.field.allowLanAccessDescription')}
            description={(
              <>
                <span className="block text-[var(--status-warning)]/85">
                  {t('settings.noatinwork.desktopNetwork.field.warning')}
                </span>
                {lanRequiresPassword || lanBlockedByMissingPassword ? (
                  <span className="block text-[var(--status-warning)]/85">
                    {t('settings.noatinwork.desktopNetwork.field.passwordRequiredWarning')}
                  </span>
                ) : null}
              </>
            )}
            ariaLabel={t('settings.noatinwork.desktopNetwork.field.allowLanAccessAria')}
          />
        </div>
        ...
        {lanUrl ? (
          <div className="typography-micro text-muted-foreground/80">
            {isDirty && !savedValue
              ? t('settings.noatinwork.desktopNetwork.hint.openAfterRestart')
              : t('settings.noatinwork.desktopNetwork.hint.openNow')}
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.noatinwork.desktopNetwork.actions.saveAndRestart')}
          </Button>
        </div>
      </div>
    </SettingsSection>

...
- App id / package: `com.noatinwork.app`; app name `NoatinWork`.
...
- **Deep links** — `noatinwork://` URL scheme; a reusable intent vocabulary (`apps/deepLinks.ts`)
...
- **iOS widgets + Control Center + Notification Service Extension** — WidgetKit extension
  (`NoatinWorkWidget`), a Control Center control, and an NSE (`NoatinWorkNotificationService`)
  that refreshes widgets from push. All share the App Group `group.com.noatinwork.app`.
...
- Extensions: `NoatinWorkWidget` (WidgetKit, deployment 17.0) and `NoatinWorkNotificationService`
  (NSE, 15.5), both hand-wired into `App.xcodeproj/project.pbxproj` and embedded via a copy phase.
- App Group `group.com.noatinwork.app` in all three targets' entitlements (app + widget + NSE).
- `Info.plist`: `CFBundleURLTypes` scheme `noatinwork`, `NSCameraUsageDescription`.
- Push entitlement (aps-environment) required.
- APNs `mutable-content: 1` (set server/relay side) wakes the NSE to refresh widgets.

### Android (`android/app`)

- `google-services.json` (committed; Firebase project `noatinwork-8bf7e`). The Google Services
...
- Apple Developer account; App IDs for the app **and** both extensions
  (`com.noatinwork.app`, `.NoatinWorkWidget`, `.NoatinWorkNotificationService`), each enabled for

...
const localOrigin = readArgValue('--noatinwork-local-origin');
const apiBaseUrl = readArgValue('--noatinwork-api-base-url');
const clientToken = readArgValue('--noatinwork-client-token');
const runtimeHeadersRaw = readArgValue('--noatinwork-runtime-headers');
const homeDirectory = readArgValue('--noatinwork-home');
const macosMajorRaw = readArgValue('--noatinwork-macos-major');
const macosMajor = Number.parseInt(macosMajorRaw, 10);
const trayEnabled = process.platform !== 'darwin' || readArgValue('--noatinwork-tray-enabled') !== '0';
...
//  - __NOATINWORK_ELECTRON__ is a shell-identity flag (no capability).
...
//  - __NOATINWORK_DESKTOP__ is the IPC channel to the main process. It is
...
const isLocalPage = currentOrigin !== 'null'
  && (currentOrigin === 'noatinwork-ui://app'
  || (localOrigin && currentOrigin === localOrigin));

// Remote pages need __NOATINWORK_LOCAL_ORIGIN__ so the HostSwitcher knows
...
if (localOrigin) {
  contextBridge.exposeInMainWorld('__NOATINWORK_LOCAL_ORIGIN__', localOrigin);
}

if (apiBaseUrl) {
  contextBridge.exposeInMainWorld('__NOATINWORK_API_BASE_URL__', apiBaseUrl);
}

if (clientToken && isLocalPage) {
  contextBridge.exposeInMainWorld('__NOATINWORK_CLIENT_TOKEN__', clientToken);
}
...
const relayHostId = readArgValue('--noatinwork-relay-host-id');
if (relayHostId && isLocalPage) {
  contextBridge.exposeInMainWorld('__NOATINWORK_RELAY_HOST_ID__', relayHostId);
}
...
if (runtimeHeadersRaw && isLocalPage) {
  try {
    ...
    if (runtimeHeaders && typeof runtimeHeaders === 'object') {
      contextBridge.exposeInMainWorld('__NOATINWORK_RUNTIME_HEADERS__', runtimeHeaders);
    }
  } catch {
  }
}
...
if (isLocalPage && homeDirectory) {
  contextBridge.exposeInMainWorld('__NOATINWORK_HOME__', homeDirectory);
}
...
if (Number.isFinite(macosMajor) && macosMajor > 0) {
  contextBridge.exposeInMainWorld('__NOATINWORK_MACOS_MAJOR__', macosMajor);
}

contextBridge.exposeInMainWorld('__NOATINWORK_ELECTRON__', {
  runtime: 'electron',
  arch: process.arch,
  trayEnabled,
});

contextBridge.exposeInMainWorld('__NOATINWORK_PLATFORM__', process.platform);
...
ipcRenderer.on('noatinwork:emit', (_evt, payload) => {
...
// ipcMain.handle('noatinwork:invoke') decides per-command what is safe
// for non-local callers (window/host-switcher ops yes, file/shell ops
// no). See COMMANDS_SAFE_FOR_REMOTE in main.mjs.
contextBridge.exposeInMainWorld('__NOATINWORK_DESKTOP__', {
  invoke: (cmd, args) => ipcRenderer.invoke('noatinwork:invoke', cmd, args || {}),
  openDialog: (options) => ipcRenderer.invoke('noatinwork:dialog:open', options || {}),
  grantFileAccess: (filePath) => ipcRenderer.invoke('noatinwork:file:grant-existing', filePath),
  openExternal: (url) => ipcRenderer.invoke('noatinwork:invoke', 'desktop_open_external_url', { url }),

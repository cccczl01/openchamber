...
const AUTO_DISCOVER_ACTION_ID = '__noatinwork_auto_discover_preview__';
...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<NoatinWorkProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<NoatinWorkProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: NoatinWorkProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: NoatinWorkProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<NoatinWorkProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: NoatinWorkProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: NoatinWorkProjectAction, toggleStopIfRunning = false) => {

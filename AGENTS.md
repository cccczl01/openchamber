# NoatinWork Agent Guide

## Purpose

NoatinWork provides shared web, desktop, VS Code, hosted-mobile, and native-mobile UI surfaces for OpenCode.
...
- `packages/web`: web surfaces, NoatinWork server, managed/external OpenCode lifecycle, and CLI.
...
- `packages/mobile`: Capacitor iOS/Android shell; bundles the mobile web surface and connects to an existing NoatinWork server.
- `packages/docs`: product documentation; not a Bun workspace.

Shared UI calls official OpenCode APIs through `@opencode-ai/sdk/v2`. NoatinWork-owned capabilities use `RuntimeAPIs`, `runtimeFetch`, and shared browser/realtime transport helpers. Server-side upstream integrations may use their owning runtime modules.

Electron starts the NoatinWork backend in-process, never as a sidecar. Development may load loopback/HMR UI; packaged builds load staged assets through `noatinwork-ui://` while the loopback server remains the API backend. Keep domain backends in web/runtime modules unless behavior is inherently native.
...
|---|---|
| Any source, dependency, export, build-config, generated-asset, package-contract, or module-ownership change | `noatinwork-change-discipline` |

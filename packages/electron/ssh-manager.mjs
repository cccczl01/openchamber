...
const SSH_STATUS_EVENT = 'noatinwork:ssh-instance-status';
...
if [[ -n "$NOATINWORK_SSH_ASKPASS_VALUE" ]]; then
  if [[ "$PROMPT" == *"assword"* || "$PROMPT" == *"passphrase"* ]]; then
    printf '%s\\n' "$NOATINWORK_SSH_ASKPASS_VALUE"
    exit 0
  fi
fi
...
const windowsAskpassScriptContent = () => `$value = [Environment]::GetEnvironmentVariable('NOATINWORK_SSH_ASKPASS_VALUE')
if ($null -ne $value) {
...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded NoatinWork health');
};
...
export class ElectronSshManager {
  ...
  authEnvironment(parsed) {
    ...
    return {
      ...
      DISPLAY: '1',
      ...(auth.sshPassword ? { NOATINWORK_SSH_ASKPASS_VALUE: auth.sshPassword.trim() } : {}),
    };
  }
  ...
  sanitizeInstance(instance) {
    ...
    return {
      ...
      remoteNoatinWork: {
        mode: instance?.remoteNoatinWork?.mode === 'external' ? 'external' : 'managed',
        keepRunning: instance?.remoteNoatinWork?.keepRunning !== false,
        ...(Number.isFinite(instance?.remoteNoatinWork?.preferredPort) ? { preferredPort: Number(instance.remoteNoatinWork.preferredPort) } : {}),
        installMethod: ['npm', 'bun', 'download_release', 'upload_bundle'].includes(instance?.remoteNoatinWork?.installMethod)
          ? instance.remoteNoatinWork.installMethod
          : 'bun',
        uploadBundleOverSsh: Boolean(instance?.remoteNoatinWork?.uploadBundleOverSsh),
      },
      localForward: {
      ...
      auth: {
        ...
        ...(this.sanitizeStoredSecret(instance?.auth?.noatinworkPassword) ? { noatinworkPassword: this.sanitizeStoredSecret(instance.auth.noatinworkPassword) } : {}),
      },
      portForwards,
    };
  }
  ...
  async issueClientToken(localUrl, noatinworkPassword) {
    const password = typeof noatinworkPassword === 'string' ? noatinworkPassword.trim() : '';
    if (!password) return '';
    ...
    const loginResponse = await fetch(new URL('/auth/session', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({
        ...
        clientLabel: 'NoatinWork Desktop SSH',
      }),
    });
    if (!loginResponse.ok) {
      throw new Error(`Configured NoatinWork UI password was rejected by forwarded server (status ${loginResponse.status})`);
    }
    ...
    const tokenResponse = await fetch(new URL('/api/client-auth/clients', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({ label: 'NoatinWork Desktop SSH' }),
    });
  ...
  configuredNoatinWorkPassword(instance) {
    const secret = instance?.auth?.noatinworkPassword;
  ...
  async currentRemoteNoatinWorkVersion(parsed, controlPath) {
    try {
      const output = await this.runRemoteCommand(parsed, controlPath, 'noatinwork --version 2>/dev/null || true');
  ...
  async installNoatinWorkManaged(parsed, controlPath, version, preferred) {
    ...
    if (preferred === 'bun') {
      if (hasBun) commands.push(`bun add -g @noatinwork-app/web@${version}`);
      if (hasNpm) commands.push(`npm install -g @noatinwork-app/web@${version}`);
    ...
    throw lastError || new Error('Failed to install NoatinWork on remote host');
  }

  async probeRemoteSystemInfo(parsed, controlPath, port, noatinworkPassword) {
    const authPayload = noatinworkPassword ? JSON.stringify({ password: noatinworkPassword }) : '{}';
    const authEnabled = noatinworkPassword ? '1' : '0';
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (noatinworkPassword && authStatus !== 200) {
          throw new Error(`Remote NoatinWork requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote NoatinWork requires UI authentication on /api/system/info; configure NoatinWork UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote NoatinWork probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async remoteServerRunning(parsed, controlPath, port, noatinworkPassword) {
    try {
      await this.probeRemoteSystemInfo(parsed, controlPath, port, noatinworkPassword);
      return true;
    } catch {
      return false;
    }
  ...
  async startRemoteServerManaged(parsed, controlPath, instance, desiredPort) {
    let envPrefix = 'NOATINWORK_RUNTIME=ssh-remote';
    const secret = this.configuredNoatinWorkPassword(instance);
    if (secret) {
      envPrefix += ` NOATINWORK_UI_PASSWORD=${shellQuote(secret)}`;
    }
    const output = await this.runRemoteCommand(parsed, controlPath, `${envPrefix} noatinwork serve --hostname 127.0.0.1 --port ${desiredPort}`);
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteNoatinWork.mode === 'external') {
      if (!instance.remoteNoatinWork.preferredPort) {
        throw new Error('External mode requires a preferred remote NoatinWork port');
      }
      const port = instance.remoteNoatinWork.preferredPort;
      this.setStatus(instance.id, 'server_detecting', 'Probing external NoatinWork server', null, null, port, false, 0, false);
      await this.probeRemoteSystemInfo(parsed, controlPath, port, this.configuredNoatinWorkPassword(instance));
      return { remotePort: port, startedByUs: false };
    }

    this.setStatus(instance.id, 'remote_probe', 'Checking remote NoatinWork installation');
    const installedVersion = await this.currentRemoteNoatinWorkVersion(parsed, controlPath);
    if (!installedVersion) {
      this.setStatus(instance.id, 'installing', 'Installing NoatinWork on remote host');
      await this.installNoatinWorkManaged(parsed, controlPath, this.appVersion, instance.remoteNoatinWork.installMethod);
    } else if (installedVersion !== this.appVersion) {
      this.setStatus(instance.id, 'updating', `Updating remote NoatinWork from ${installedVersion} to ${this.appVersion}`);
      await this.installNoatinWorkManaged(parsed, controlPath, this.appVersion, instance.remoteNoatinWork.installMethod);
    }

    this.setStatus(instance.id, 'server_detecting', 'Detecting managed NoatinWork server');
    let remotePort = instance.remoteNoatinWork.preferredPort || null;
    let startedByUs = false;
    if (remotePort && !(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredNoatinWorkPassword(instance)))) {
      remotePort = null;
    }
    if (!remotePort) {
      this.setStatus(instance.id, 'server_starting', 'Starting managed NoatinWork server');
      const desiredPort = instance.remoteNoatinWork.preferredPort || randomPortCandidate(instance.id);
      remotePort = await this.startRemoteServerManaged(parsed, controlPath, instance, desiredPort);
      startedByUs = true;
    }
    if (!(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredNoatinWorkPassword(instance)))) {
      throw new Error('Managed NoatinWork server failed to become reachable');
    }
  ...
  async disconnectInternal(id, reportIdle) {
    ...
    if (session) {
      if (session.startedByUs && session.remotePort && session.instance.remoteNoatinWork.mode === 'managed' && !session.instance.remoteNoatinWork.keepRunning) {
  ...
  async connectBlocking(instance) {
    ...
    const clientToken = await this.issueClientToken(localUrl, this.configuredNoatinWorkPassword(instance));
